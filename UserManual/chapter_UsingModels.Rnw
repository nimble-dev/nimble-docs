...
one or more nodes or variables. While this functionality can be used
from R, its primary use is for determining algorithm behavior within the setup code of
\cd{nimbleFunctions}. We briefly demonstrate some of the functionality here,
...
name of the distribution. In particular, one can determine whether a distribution
...
You can query whether a node is flagged as data using the \cd{isData} method applied to one or more nodes:
<<isData>>=
pump$isData('x[1]')
...
\subsection{\cd{getNodeValueExpr}, \cd{getNodeType}, \cd{getNodeDistribution} and \cd{getNodeParamExpr}}
\label{sec:moreNodeInfo}
These functions provide further information about nodes that can be useful when writing setup code.

<<getNodeValueExpr>>=
pump$getNodeValueExpr('alpha')
pump$getNodeValueExpr('lambda[1]')
pump$getNodeType('beta')
pump$getNodeType('lambda[1]')
pump$getNodeDistribution('theta')
pump$getNodeDistribution('lambda') ## NA because node is deterministic
pump$getNodeParamExpr('alpha', 'rate')
@ 


\subsection{\cd{isDiscrete}, \cd{isBinary} and \cd{isTruncated}}
\label{sec:evenMoreNodeInfo}
Finally, the outputs of the following might also be useful in setup code.

<<>>=
pump$isBinary('alpha')
pump$isDiscrete('alpha')
pump$isTruncated('alpha')
@ 
