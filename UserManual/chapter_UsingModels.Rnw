...
We can determine the set of nodes contained in one or more nodes or variables using \cd{expandNodeNames}, illustrated here for an example with multivariate nodes. The \cd{returnScalarComponents} argument also allows us to return all of the scalar components of multivariate nodes. 
...
object, taking a vector of node names as an argument\footnote{Standard usage is \cd{model\$calculate(nodes)} but calculat(model, nodes) is synonymous.}.   If there is
...
\subsection{\cd{simNodes}, \cd{calcNodes}, and \cd{getLogProbNodes}}
\label{sec:cdsimn-cdcalcn-cdget}

{\color{red} under discussion: how to clarify the point of these and where to put this text}
...
takes a model and a character string of node names 
 ...
 the calculations must be up to date (from a calculate call) for \cd{getLogProbNodes}
...
{\color{red} I'm somewhat inclined to remove this - this feels like access to private member data in OOP - we provide getLogProb as the public accessor, so is there a need to even tell users about this? Or if not remove, we might mention when we talk about getLogProb that what it does is return the relevant current value(s) in logProb\_variableName.}
