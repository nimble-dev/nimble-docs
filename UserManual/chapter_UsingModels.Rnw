...
\section{The variables and nodes in a NIMBLE}
\label{sec:accessing-variables}

Section \ref{sec:nodes-and-variables} describes the definitions of variables and nodes in a NIMBLE model and discusses how to determine and access the nodes in a model and their dependency relationships. 

{\color{red} need to work on overlap between stuff below here and what is in chapter 6}
...
Model variables and nodes can be accessed and set just as in R using \cd{\$} and
\cd{[[ ]]} as discussed in Section \ref{sec:accessing-nodes}.

Variables and nodes that contain more than one value can also be subindexed, e.g.,
<<subindexing, eval=FALSE>>=
model[['z[1:5']][2:3]
model[['z[1:5']][2:3] <- vals 
@ 

{\color{red} check that we allow subindexing on LHS of run code}
