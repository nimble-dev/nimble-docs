...
densities) that have already been calculated, and the log probability
...
\item[calculateDiff]  \cd{calculateDiff} is identical to
  ...
  the one that was previously stored.  
\end{description}

There are two ways to access \cd{calculate}, \cd{calculateDiff},
\cd{simulate}, and \cd{getLogProb}.  The primary way is via the
functions with those names, which can use arbitrary collections of
nodes.  In that case, \cd{calculate} and \cd{getLogProb} return the
sum of the log probabilities from each node, while \cd{calculateDiff}
returns the sum of the new values minus the old values.  The other way
is to directly access the corresponding function for each node in a
model.  Normally you'll use the first way, but we'll show you both.
...
TODO: CHECK IF THAT LAST POINT IS STILL TRUE

Use of \cd{calculate}, \cd{calculateDiff} and \cd{getLogProb} is simliar to \cd{simulate},
except that they return the sum of the log probabilities (densities)
of the nodes requested, and they have no \cd{includeData} argument.

\subsection{Direct access to each node's functions}
\label{sec:direct-access-each}

Access to the underlying \cd{calculate}, \cd{calculateDiff}, \cd{simulate}, and
...
a \cd{nimbleFunction} (\ref{cha:progr-with-models}).  
...
NIMBLE (\ref{sec:introduced-nodes}).
...
``descendents'') in a model. There are a variety of arguments that
...
stochastic node on all edges of the graph.  This is what would be
...
TODO: CHECK IF FOLLOWING IS STILL TRUE
...
TODO: IT LOOKS LIKE WE DON'T EXPLAIN expandNodeNames anywhere?
...
In the NIMBLE framework, \cd{modelValues} are containers designed for
...
three-dimensional array) from the same dimension\footnote{In v\ver,
  NIMBLE is limited to four dimensions.}. The simplest way to build a
...
rows. As you can see, the rows are stored as elements of a list.
  ...
  \cd{modelValuesSpec()} function, like this:
%%   \begin{itemize}
...
The arguments to \cd{modelValuesSpec} are matching lists of variable
names, types, and sizes. See \cd{help(modelValuesSpec)} for more
...
the model member function \cd{newModel} that will create a new copy of
the model from the same model definition
(\ref{sub:multiple-instances}). This new model can then be used with
