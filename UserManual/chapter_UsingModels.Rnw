...
be used from R, the primary use of getting information and operating a
model occurs when writing nimbleFunctions (see Chapter \ref{cha:progr-with-models}). 
...
on a model, including setting node or variable values, would generally be done in \nm{run} code.
...
{\color{red} the content on getNodeNames (but not expandNodeNames) repeats stuff from Chap. 6; thoughts?}
{\color{red} one idea is to just have getNodeNames in its most basic usage shown in Chap 6 and more extended usage here}

One can determine the variables in a model using \cd{getVarNames} and the  nodes in a model using \cd{getNodeNames}, with optional arguments allowing you to select only certain types of nodes. 

<<getVarAndNodeNamesPump>>=
@ 

You can see one lifted node (see next section) for the Cholesky decomposition of
\cd{cov}, and the two multivariate normal nodes.

We can determine the set of nodes contained in one or more nodes or variables using \cd{expandNodeNames}. The \cd{returnScalarComponents} argument also allows us to return all of the scalar components of multivariate nodes. 
...
{\color{red} this repeats stuff from Chap. 6; thoughts?}
...
Model variables and nodes can be accessed and set using \cd{[[ ]]}, but there are some limitations on what can be compiled relative to the syntax  discussed in Section \ref{sec:accessing-nodes}. 
...
{\color{red} this is what I'm envisioning but some of this is not compiling - perhaps Perry or Daniel can set me straight and flesh this out}
...
nfGen <- nimbleFunction(
    ...
    run = function(vals = double(1)) {
        ...
        model[[node2]][1] <<- runif(1) # only an element of node2 can be assigned a scalar
...
In addition, one can use \cd{values} to get or set the value(s) of one more nodes in a vectorized fashion, as discussed in Section \ref{sec:getting-setting-more}.
...
{\color{red} CJP commented elsewhere that not clear if these nf's
  could be replaced with an extra argument to
  calculate/simulate/getLogProb that allows on to specify that
