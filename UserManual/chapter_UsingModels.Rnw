...
returns the sum of the new values minus the old values. These can be
used as member functions of the model object, or equivalently they can
take the model object as the first argument.  The other way
...
\item \cd{simulate(model, nodes)} is equivalent to
  \cd{model\$simulate(nodes)}. You can use either.
...
\cd{getLogProb} functions built by NIMBLE can be obtained (in R only,
not in compiled code) as follows:
