...
TODO: I don't think this bit about parse/deparse is needed given the above text - CJP
...
\item[calculateDiff]  \cd{calculateDiff} is identical to
  ...
  the change in the log probability, such as in Metropolis-Hastings. 
\end{description}

There are two ways to access \cd{calculate}, \cd{calculateDiff},
\cd{simulate}, and \cd{getLogProb}.  The primary way is via the
functions with those names, which accept arbitrary collections of
nodes as input.  In that case, \cd{calculate} and \cd{getLogProb} return the
