...
<<lw_Filter_Chunk, results = "hide">>=
  ...
  cLWF$run(parNum)
@
  Below we give an example of building and running the ensemble Kalman filter, which can sample from the posterior distribution of latent states. Additional information can be found with \cd{help(buildENKF)}.
...
<<getNormConst>>=
newPump2 <- newPump$newModel(replicate = T)
