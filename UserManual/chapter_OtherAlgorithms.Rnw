...
@
  ...
  The first argument to the MCEM, \cd{model}, is a NIMBLE model, which can be
...
The  \cd{mcmcControl} argument will be passed to \cd{configureMCMC} to define the MCMC to be used.
 ...
 be optimized, using NIMBLE's getBounds() function.  It is also possible for a user to manually specify constraints via the \cd{boxConstraints} argument.
