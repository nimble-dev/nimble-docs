...
OpenBUGS and/or JAGS, so we start by summarizing how NIMBLE is similar
and different to those before documenting NIMBLE's version of BUGS
...
For example, a simple linear regression with four observations is
...
When a variable appears only on the right-hand side, it must be
provided in the \cd{data} or \cd{constants} argument to
\cd{nimbleModel}, covered in chapter \ref{cha:building-models}.  
...
\cd{xbar <- mean(x[])} (if \cd{x} is a vector) or \cd{xbar <-
 mean(x[,])} (if \cd{x} is a matrix) for NIMBLE.  \footnote{In \cd{nimbleFunctions}
  ...
  not necessary for multivariate objects.}.  Ways to provide NIMBLE with dimensions of \cd{x} in such cases, if
necessary, is covered in section \ref{}.
...
Supported functions from BUGS and JAGS are listed in Section
\ref{subsec:BUGS-lang-fxns}.  Support for more general R expressions
is covered in Chapter \ref{cha:progr-with-models} about programming
...
See section \ref{} to learn how to write new distributions using nimbleFunctions.
...
parameterizations. Available parameterizations is listed in Table \ref{table:distributions-alternates}.
...
available operators and functions. {\color{red} This table needs updating.}
...
{\color{red} This table needs updating, at least for eigen and svd.}
...
Note that NIMBLE does not provide an automatic way of initializing the additional node (\cd{logit\_p} in this case), which is a parent node of the explicit node (\cd{p[i]}), without explicitly referring to the additional node by the name that NIMBLE generates. 
...
\item \cd{x $\sim$ T(dnorm(0, sd = 10), 0, a)}, 
  ...
  \cd{a} (inclusive of 0 and a).  Either boundary may be omitted or may be another node, such as \cd{a} in this example.  The first form is compatible with JAGS, but in NIMBLE it can only be used when reading code from a text file.  When writing model code in R, the second version must be used.  
...
<<understandingNimbleConcepts, eval=FALSE>>=
@ 
