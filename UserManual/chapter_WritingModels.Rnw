...
{\color{green} Add gentle introduction to deterministic and stochastic
  nodes here, with some explanation of how to think about a graph.}
...
{\color{green} Change this to note that providing indices when needed
  is covered in next chapter.}
...
should use all elements of a variable, similarly to R).}. {\color{red}
