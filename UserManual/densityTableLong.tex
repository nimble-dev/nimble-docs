...
%      Double  & \verb+ddexp(mu,tau)+ & 
 ...
 %     & $n > 0$, $m > 0$ \\
      Flat (improper) & \cd{dflat()} & $\propto 1$ & & \\
 ...
 %     gamma       & $\lambda >0$, $b > 0$, $r > 0$ \\
      Half Flat (improper) & \cd{dflat()} & $\propto 1$ & $0$ & \\
 ...
 %     hypergeometric & $0 \leq n_i$, $0 < m_1 \leq n_+$  \\
      Wishart (FIXME - check) & \cd{dwish(R = R, df = k)} &
      \multirow{2}{*}{
        $\frac{\textstyle |x|^{(k-p-1)/2} |R|^{k/2} \exp\{-\text{tr}(Rx/2)\}}
               {\textstyle 2^{pk/2} \pi^{p(p-1)/4} \prod{i=1}{p} \Gamma((k+1-i)/2)}$
      } \\
      & $R \; p \times p$ pos. def., $k \geq p$ \\
      Inverse Wishart (FIXME - check) & \cd{dinvwish(S = S, df = k)} &
      \multirow{2}{*}{
        $\frac{\textstyle |x|^{-(k+p+1)/2} |S|^{k/2} \exp\{-\text{tr}(Sx^{-1}/2)\}}                           {\textstyle 2^{pk/2} \pi^{p(p-1)/4} \prod{i=1}{p} \Gamma((k+1-i)/2)}$
      } \\
      & $S \; p \times p$ pos. def., $k \geq p$ \\
