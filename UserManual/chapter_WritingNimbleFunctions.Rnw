...
\chapter{Writing \nm{nimbleFunctions}  that interact with models}
...
returned value\footnote{Normally this is the value of the last
...
\item \nm{Setup} code is provided as a regular R function, but the
  ...
  a modelValues object or a modelValues configuration, or another nimbleFunction.  The \cd{setup} code,
...
\item \nm{Run} code is provided in the NIMBLE language.  This is
  ...
  \item There is always a primary function, given as an argument
    called \nm{run}\footnote{This can be omitted if you don't need it.}.
...
in a model, set up modelValues objects, set up (nested) nimbleFunctions 
...
numeric values and modelValues can be. For example:
...
In Section \ref{sec:nimble-lang-comp-simple} we described the functionality of the NIMBLE language that could be used in run code in which models were not used. When writing nimbleFunctions that use models, one can use that functionality already described as well as the functionality discussed in this section. 
...
{\color{red} check on outcome of discussion on fixed ranges}
...
\begin{itemize}
  \item use of \cd{[[ ]]} is restricted to use with a single node (potentially a non-scalar node), and
...
One can also use indexing, but one can only have a single index, and not a range of indices:

<<eval=FALSE>>=
## get values from a set of model nodes into a vector
i <- 2
P <- values(model, nodes[i])
## or put values from a vector into a set of model nodes
values(model, nodes[i]) <- P
# P <- values(model, nodes[1:2] # won't compile
@ 
...
{\color{red} need to determine if/how obsolete this is given nimbleLists; if text is kept, make clear when one would use this instead of a nimbleList}
...
my_RW <- nimbleFunction(
    ...
    run = function() {
        model_lp_initial <- getLogProb(model, calcNodes) ## initial model logProb
        proposal <- rnorm(1, model[[target]], scale)     ## generate proposal
        model[[target]] <<- proposal                     ## store proposal into model
        model_lp_proposed <- calculate(model, calcNodes) ## proposal model logProb
...
\begin{itemize}
  \item Include the contains statement \cd{contains = sampler\_BASE}.
    \item The \cd{setup} function must have the exact four arguments
      ...
      \item The \cd{run} function must accept no arguments, and have
        ...
        \item Have a member method called \cd{reset}, which takes no arguments 
