...
{\color{red} We may want to move the comments on the requirements into standard text; also don't samplers need to be named 'sampler\_{name}'? More extended user-defined samplers are in the user\_sampler module of our outreach; should we put more of that here?}
