...
\item \nm{Run} code is provided in the NIMBLE language.  This is
...
\item[vectors of node names] Often these are obtained from the
  ...
  described in Section \ref{sec:accessing-variables}.
\item[modelValues objects] These are discussed in Sections \ref{sec:modelValues-struct} and \ref{sec:access-model-modelv}.
\item[nimbleList definitions]  It can be useful to create \cd{nimbleList} definitions in setup code.  New instances of \cd{nimbleList}s can then be created from this definition in either setup or run code. See Section \ref{sec:nimbleLists} for more information.
...
Functionality for getting information about a model and the nodes in a model (see Sections \ref{sec:accessing-variables}-\ref{sec:nodeInfo}) are also often used in setup code.
...
In Chapter \ref{cha:RCfunctions} we described the functionality of the NIMBLE language that could be used in run code in which models were not used. When writing nimbleFunctions that use models, one can use that functionality already described as well as the functionality discussed in this section. 
...
single elements indexed by a variable, or ranges denoted with \cd{:}, as well as expressions that evaluate to logical vectors of length equal to the number of nodes.  For example, all of the follwing are allowed.
...
Use of \cd{values} is explained in the next section.

{\color{red} Perry, please check this section}
...
{\color{red} This deserves a careful pass from Perry or Daniel to check that I've gotten right what we allow and that I'm not omitting any important syntax that we do allow.}

Model variables and nodes can be accessed and set using \cd{[[ ]]} in run code, but there are some limitations on what can be compiled relative to the syntax  discussed in Section \ref{sec:accessing-nodes}. 

Here is an example that illustrates getting and setting of nodes and subsets of nodes.  Note the following:

\begin{itemize}
  \item use of \cd{[[ ]]} is currently restricted to use with a single node (potentially a vector node)\footnote{We plan to generalize this in an upcoming version.}, and
 \item one should use the \cd{<<-} global assignment operator to assign into model nodes. 
\end{itemize}

In this example, we illustrate assignment based on double bracket indexing. Note that the dimensions of variables created in run code will be determined at run time based on the dimensions of \cd{node1} and \cd{node2}. In some cases (e.g., assignment of the \cd{runif(1)} scalar value), this means the programmer needs to plan in advance for the dimensionality of objects provided in setup code. This run code assumes that \cd{node2} will always be a vector node.

{\color{red} Uggh, is there a better way to say that? Stated in that way, it does highlight there are tricky issues here...}
...
nfGen <- nimbleFunction(
    setup = function(model, node1, node2) {
    },
    run = function(vals = double(1)) {
        tmp0 <- model[[node1]]    # tmp0 will be a scalar
        tmp1 <- model[['sigma']]  # allowed but not model-generic
        tmp2 <- model[[node2]]    # tmp2 will be a vector
        tmp3 <- model[[node2]][1]  # tmp3 will be a scalar
        model[[node1]] <<- runif(1) # node1 is scalar so can be assigned a scalar
        model[['sigma']] <<- runif(1) # allowed but not model-generic
        model[[node2]][1] <<- runif(1) 
         # only an element of node2 can be assigned a scalar 
        model[[node2]][1:3] <<- vals[1:3] 
         # allowed but may not be model-generic, and could cause run-time error
        returnType(double(1))
        out <- model[[node2]]
        return(out)
    }
...
Use of \cd{[[ ]]} allows one to programmatically access a node based on a character variable containing the node name; this character variable would generally be set in setup code. In contrast, use of \cd{\$} hard codes the variable name and would not generally be suitable for nimbleFunctions intended for use with arbitrary models. Furthermore one can only use \cd{\$} in very limited ways.


{\color{red} do we want to show any use of \$?}
...
into \cd{P}, and the second would to the inverse.  In both cases, values
...
<<eval=FALSE>>=
\cd{Y <- A \%*\% values(model, nodes) + b}
@ 


Note that \cd{values} returns a 1-d vector and expects a 1-d vector when used on the left-hand side of an assignment. This means that one needs to do some extra work to use scalars with \cd{values}. For example:

<<eval=FALSE>>=
tmp <- numeric(1)
tmp[1] <- rnorm(1)
values(model, nodes[1]) <- tmp
# values(model, nodes[1]) <- rnorm(1)  # won't compile

out <- values(model, nodes[1])
returnType(double(0))
return(out[1])
# return(out)  # won't compile unless one has returnType(double(1))
@ 

One can also use indexing in the same manner as discussed for \cd{calculate} and related functions in Section \ref{sec:driv-models:-calc}.

{\color{red} PdV - check this last statement}
...
\cd{1:length(myFunList)} if the length of \cd{myFunList} is greater than zero
it is an empty sequence if the length is zero.
...
{\color{red} Should we keep this in light of nimbleLists? I tried to intro this section such that it's clear one can use nimLists, but we may want to make clear when one would use this instead of a nimbleList}
...
{\color{red} I don't understand the comments about 'will be a scalar if all sizes are 1' in the next code chunk}
...
{\color{red} compiled usage is NOT working here - returning NULL; this was true in v0.6-3 manual as well...}

<<eval=FALSE>>=
valueInCompiledNimbleFunction(CmyDataNF, 'X')
@ 

{\color{red} valueInCompiledNimbleFunction is giving an error: Error in eval(call(".Call", nimbleUserNamespace$sessionSpecificDll$getModelObjectPtr, )) : 
  object 'basePtr' not found
when the useMultiInterface is set to TRUE}
