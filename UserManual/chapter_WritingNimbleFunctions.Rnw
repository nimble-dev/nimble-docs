...
Even non-sequential index vectors should work.  Could we simple say:

``Most R-like indexing of a node vector is allowed within the
argument to calculate, calculateDiff, simulate, or getLogProb.  For
 example, all of the following are allowed:... [as you have above]
...
OK, I have revised this below. -PdV.  I think the model-generic issue
is distracting here.  I will try to make a simple statement about the
vector/scalar issues.}

% Model variables and nodes can be accessed and set using \cd{[[ ]]} in run code, but there are some limitations on what can be compiled relative to the syntax  discussed in Section \ref{sec:accessing-nodes}.  % not sure what you were talking about here. -PdV
...
\begin{itemize}
  ...
  \item In fact, \code{node} can be a node-name-like character string, even if it is not actually a node in the model.  See example 4 in the code below.
  \item One can also use \cd{model\$nodeName}, with the caveat that \cd{nodeName} can't be a variable and so would only make sense for a nimbleFunction written for models known to have a specific node.
...
nfGen <- nimbleFunction(
    setup = function(model) {
        ...
        notReallyANode <- 'y[2:4]'  ## y[2:4] is allowed even though it is not a node!
    },
    run = function(vals = double(1)) {
        ...
        model[[node1]] <<- runif(1) # 5. node1 is scalar so can be assigned a scalar
        model[[node2]][1] <<- runif(1) 
                 # 6. an element of node2 can be assigned a scalar 
        model[[node2]] <<- runif(length(model[[node2]]))
                 # 7. a vector can be assigned to the vector node2
...
can be achieved by wrapping a scalar in \code{c()} when necessary.
For example:

<<eval=FALSE>>=
values(model, nodes[1]) <- c(rnorm(1)) # c(rnorm(1)) creates vector of length one
# values(model, nodes[1]) <- rnorm(1)  # won't compile because rnorm(1) is a scalar
...
I think I simplified the above bit. -PdV}
...
{\color{red} TWO COMMENTS: 1. We shouldn't encourage the style of
  creating modelValuesConf in setup code, because that might imply it
  could be different in different specializations, but it can't.  It's better to
  pass the conf as a setup argument. 2. It seems confusing to give
  examples called ``setupFunction'' and ``runFunction''.  Can we use
  ``setup'' and ``run'' even if they are not shown in a full call to ``nimbleFunction''?}
...
I like your introduction.  Let's keep the section but I'm ok if we
reduce it.  I fixed the problems below. -PdV}
...
There were a couple of problems: There was a bug.  And the option name
has changed from what was in the chunk code (but not the text above)
(and default may have changed since we last checked this section.)
I'd be ok if we just want to reduce or omit some of this detail, but I
like the idea of including this way to use a nimbleFunction. -PdV}
