...
appear as variables in the model and be flagged as data, but will not
...
mu2 ~ dnorm(0, 1) 
ind <- mu1 + mu2 > 0
const ~ dconstraint(ind)
@ 

One would then specifies \cd{const} to be data and to have value 1. This is equivalent to a half-normal distribution on the half-plane $\mu_1 + \mu_2 > 0$. However, note that this equivalence only holds when conditioning on \cd{const}. If one simulates from the model using the \cd{simulate} functions, \cd{ind} will merely indicate whether the condition  $\mu_1 + \mu_2 > 0$ is satisfied given the current values of \cd{mu1} and \cd{mu2}. If the condition is not satisfied, then \cd{ind} and \cd{const} will be 0 and the log probability of \cd{const} (and therefore of the model as whole) will be $-\infty$.

Note that in the current version of NIMBLE one must specify the condition as a separate line. One cannot use an expression as the parameter of \cd{dconstraint}. One can use \cd{\&} and \cd{|} to construct compound comparisons in the definition of the indicator, e.g., \cd{ind <- mu1 + mu2 > 0 \& mu1 > 0}. 
...
ind <- alpha1 <= alpha2 & alpha2 <= alpha3
const ~ dconstraint(ind)
@ 
...
<<constraint-caution1, eval=FALSE>>=
alpha1 ~ dnorm(0, 1)
...
Similarly, 
<<constraint-caution2, eval=FALSE>>=
mu1 ~ dnorm(0, 1)}
mu2 ~ dnorm(0, 1) T(-mu1, )
@ 
...
As of Version 0.4, NIMBLE now allows you to define your own functions and distributions (as \cd{nimbleFunctions} of course!) for use in BUGS code. This introduces enormous flexibility for users. Thus you are no longer limited to the functions and distributions listed in Section \ref{sec:more-details-nimble}.

Further, while NIMBLE at the moment does not allow the use of random indices, such as is common in clustering contexts, you may be able to analytically integerate over the random indices, resulting in a mixture distribution that you could implement as a user-defined distribution. This also would allow you to implement the dnormmix distribution provided in JAGS.

This also allows you to specify models of reduced dimension, which can ease computations in various algorithms, such as MCMC. For example, one can write a Dirichlet-multinomial distribution that allows one to avoid working directly with the underlying latent probabilities in models with multinomial likelihoods and Dirichlet prior on the multinomial probability vector.
...
\begin{enumerate}
  \item define density ('d') and simulation ('r') \cd{nimbleFunctions} without setup code.
   \item register the distribution using the \cd{registerDistributions} function, and
     \item use your distribution in BUGS code.
\end{itemize}

Here is an extended example of providing a univariate distribution (simply duplicating the exponential distribution already provided by NIMBLE) and a new multivariate distribution, the Dirichlet-multinomial, not provided by NIMBLE. Note that we can use truncation with the user-provided exponential distribution because we have provided distribution and quantile functions.
...
When you write your user-defined distribution-related functions, you should write them to take as input the parameters for a single parameterization, which will be the standard parameterization that NIMBLE will use. If you would like to allow for multiple parameterizations, you can do this via the Rdist element of the list provided to registerDistributions as discussed below.
...
\begin{itemize}
  \item Your distribution-related functions must have names that begin with 'd', 'r', 'p', and 'q' and the name of the distribution must not be identical to any of the NIMBLE-provided distributions \footnote{If you wanted to create your own, say, \cd{dmnorm}, you just need to use a different name.}.
    \item Your functions must have the same name as the \cd{BUGSdist} input to \cd{registerDistributions} or as the \cd{Rdist} if you allow the BUGS name to differ from the name as implemented in the \cd{nimbleFunctions}. 
  ...
  \item Your distribution-related functions must take as arguments the default parameters, starting as the second argument and in the order used in the parameterizations in the \cd{Rdist} argument to registerDistributions or the \cd{BUGSdist} argument if there are no alternative parameterizations. 
  \item All density functions must have as their last argument \cd{log_value} and implement return of the log probability density. NIMBLE never passes \cd{log_value = 1}, though of course we recommend you implement this simple calculation. Unfortunately, you cannot name this arguments \cd{log} as in R density functions as \cd{log} is used as the logarithm function in our generated C++ code.
  \item All distribution and quantile functions must have their last two arguments be (in order), \cd{lower_tail} and \cd{log_p}. These functions must be able to properly do their computation for \cd{lower_tail = 1} (i.e., TRUE) and \cd{log_p = 0} (i.e., FALSE), as these are the inputs we use when working with truncated distributions. It is your choice whether you implement the necessary calculations for other combinations of these inputs, but if you do not we recommend you print a warning \footnote{Unfortunately, we do not yet have error-handling in \cd{nimbleFunctions}.}. Unfortunately, you cannot name these arguments \cd{lower.tail} or \cd{log.p} as the use of a period conflicts with object-oriented code syntax when we create and compile C++ code.
\end{itemize}


Next we provide basic information on the structure of the distribution information passed to \cd{registerDistributions}. The single argument should be a list of lists, with each element a list providing information about a single user-defined distribution. Full details are given in the help information for \cd{registerDistributions}. Also note that additional examples can be seen in the R/distributions_inputList.R file for the default NIMBLE distributions in the source code for the NIMBLE package or on our Github repository. 
...
\item \cd{Rdist} should provide a vector of potential reparameterizations, including reordering of parameters. The distribution name should match that of the nimbleFunctions you have written. 
\item \cd{altParams} is optional but is intended to allow for back-calculation of alternative parameters from default parameters. This is used in our MCMC conjugacy processing but is not yet relevant for user-supplied distributions as we don't provide for conjugacy for user-defined distributions.
\item \cd{discrete} is a logical value indicating whether the distribution is that of a discrete random variable
\item \cd{pqAvail} indicates if you have provided distribution and quantile functions that can be used for truncated versions of your distribution.#' }

You may notice the presence of a \cd{types} element in the list of NIMBLE-provided distributions that has information on type and dimension. This syntax is in flux at the moment and there is no need to provide \cd{types} as part of your input list to \cd{registerDistributions}. 
...
code <- BUGScode({
                     ...
                     dmu[1:3] <- vdbl(mu[1:3])
                     theta[1:3] ~ dmnorm(mn[1:3], prec[1:3, 1:3])
