...
is.censored[i] $\sim$ dinterval(t[i], c[i])
t[i] $\sim$ dweib(r, mu[i])
@ 
...
[Chris still needs to edit this]

NIMBLE also provides functionality for specifying constraints via the \cd{dconstraint} syntax. While this can be used for a variety of types of constraints, one specific usage is for specifying ordered parameters. In addition one can use \cd{dconstraint} to specify priors restricted to subdomains more flexibly than using \cd{T(,)}. 

The basic syntax specifies a node to follow \cd{dconstraint(binaryValue)} where \cd{binaryValue} is an indicator variable whose value indicates whether a given condition is satisfied. For example, we could specify that the sum of two parameters must be positive with:
\newline
\cd{mu1 $\sim$ dnorm(0, 1)} \newline
\cd{mu2 $\sim$ dnorm(0, 1)} \newline
\cd{ind <- mu1 + mu2 > 0} \newline
\cd{const $\sim$ dconstraint(ind)}

where one then specifies \cd{const} to be data and to have value 1. This is equivalent to a half-normal distribution on the half-plane $\mu_1 + \mu_2 > 0$.

Note that at the moment one must specify the condition as a separate line. One cannot use an expression as the parameter of \cd{dconstraint}. One can use \cd{\&} and \cd{|} to construct compound comparisons in the definition of the indicator, e.g., \cd{ind <- mu1 + mu2 > 0 \& mu1 > 0}. 

\paragraph{Ordering}

To specify an ordering of parameters, such as $\alpha_1 < \alpha_2 < \alpha_3$ one can use \cd{dconstraint} as follows: \newline
\cd{ind <- alpha1 < alpha2 \& alpha2 < alpha3}\newline
\cd{const $\sim$ dconstraint(ind)}

Note that this is an a posteriori ordering in that the constraint is considered to be an aspect of the data. If one is doing MCMC, this will achieve the desired ordering in the posterior. However, if one simulates from the model (i.e., sampling from the prior), then in some samples the constraint will be violated, \cd{ind} and \cd{const} will be 0 and the log probability of the model will be $-\infty$.

NIMBLE does not support the JAGS \cd{dsort} syntax.

\paragraph{Cautions}

Note that if one uses the truncation syntax to try to specify a constraint, one will not specify the prior that one might think. 

E.g., for prior ordering,\newline
\cd{alpha1 $\sim$ dnorm(0, 1)}\newline
\cd{alpha2 $\sim$ dnorm(0, 1) T(alpha1, )}\newline
\cd{alpha3 $\sim$ dnorm(0, 1) T(alpha2, )}

does not treat the three parameters in a symmetric fashion but instead has a marginal prior on \cd{alpha1} that is standard normal and then constraints $\alpha_2$ to have a truncated normal distribution. This is not equivalent to a symmetric prior on the three alphas that puts 0 mass on values that are not in order.

Similarly, \newline
\cd{mu1 $\sim$ dnorm(0, 1)}\newline
\cd{mu2 $\sim$ dnorm(0, 1) T(-mu1, )}
...
One should be cautious about constraints that would make it hard for an MCMC or optimization to move through the parameter space (such as equality constraints that involve two or more parameters). For such restrictive constraints, general purpose algorithms that are not tailed to the constraints are likely to fail. Also, one should of course always initialize a model such that the constraints are atisfied. Relying on NIMBLE's default of sampling from the prior does not ensure any constraints are satisfied.
