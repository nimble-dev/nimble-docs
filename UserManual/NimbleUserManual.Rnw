...
Here is an extended example of providing a univariate distribution (simply duplicating the exponential distribution already provided by NIMBLE) and a new multivariate distribution, the Dirichlet-multinomial, not provided by NIMBLE.
...
As discussed in more detail in the help information for \cd{registerDistributions}. Also note that additional examples can be seen in the R/distributions_inputList.R file for the default NIMBLE distributions in the source code for the NIMBLE package or on our Github repository. 
\cd{BUGSdist} should specify the name of the distribution to be used in BUGS and list all of the possible parameters from all parameterizations.
\cd{Rdist} should provide a vector of potential reparameterizations, including reordering of parameters. The distribution name should match that of the nimbleFunctions you have written. 
\cd{altParams} is optional but is intended to allow for back-calculation of alternative parameters from default parameters. This is used in our MCMC conjugacy processing but is not yet relevant for user-supplied distributions as we don't provide for conjugacy for user-defined distributions.
\cd{discrete} is a logical value indicating whether the distribution is that of a discrete random variable
\cd{pqAvail} indicates if you have provided distribution and quantile functions that can be used for truncated versions of your distribution.#' }
#' \item{\code{altParams}} {
