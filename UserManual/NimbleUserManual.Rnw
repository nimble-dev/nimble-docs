...
computationally-intensive methods.  This is the very first version,
\ver (v\ver). You can do quite a bit with it, but you can also expect it to be rough
...
\item One instance, or ``specialization'', \cd{simNodesTheta1to5}, has
  ...
  the setup function.  What is returned is a function that will
  execute the \cd{run} code.
\end{enumerate}
...
BUGS language.  However, for this first release, there are some BUGS
...
\item[code] This is R code for the BUGS model.  With just a few
  ...
  \cd{modelCode()}, the synonym \cd{BUGScode()}, or simply the R
  function \cd{quote()}.
...
\item[dimensions] This is a named list of vectors of the sizes of
  ...
  variables automatically, but in case with blank index fields,
...
pumpMissSpec <- MCMCspec(pumpMiss)
...
niter <- 1000
set.seed(0)
Cobj$pumpMissMCMC(niter)
samples <- as.matrix(nfVar(Cobj$pumpMissMCMC, 'mvSamples'))
...
\item An alternative parameterization is one that must be converted
...
of \cd{gamma}, the \nm{BUGS} parameterization is also an alternative parameterization.
...
scale) is the default. This happens to  match R's order of parameters,
...
prefix  ``logProb\_'' that matches the variable's size.  For example
...
three-dimensional array) of the same dimension\footnote{In v\ver,
...
be the types created in C++ code during compilation, so they should be specified carefully.
  ...
  objects it uses are also compiled.  See documentation for \cd{nfVar}
  for more about how to access setup variables in nimbleFunctions, including
  modelValues. A NIMBLE model always contains a
  ...
  Its compiled mv is then accessed with \cd{nfVar}.
...
cSimManyXY(m = 5) # simulating 5 times
cCalcManyXDeps(saveLP = TRUE) # calculating 
cGetLogProbMany() #
@
...
The  \cd{mcmcControl} argument will be passed to \cd{MCMCspec()} to define the MCMC to be used.
...
executes it, and returns another function, \cd{logProbCalcPlusA}.  In this case, the
...
The function \cd{logProbCalcPlusA}, returned by \cd{logProbCalcPlus},
is permanently bound to the results of the processed \cd{setup}
function.  In this case, \cd{logProbCalcPlusA} takes a scalar input value, \cd{P},
...
using using \cd{nfVar()}, which is introduced in more detail below.  In
...
\cd{nfMethod}, as illustrated in the following.
...
\cd{nfVar} and \cd{nfMethod}, respectively.  These are demonstrasted next
