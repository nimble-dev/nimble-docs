...
\begin{figure}[!h]
  ...
  \end{center}
  \end{figure}
  \href{http://r-nimble.org}{R-nimble.org}
}
\maketitle
...
\part{Introduction and building Models in NIMBLE}
...
\Sexpr{knit_child('chapter_IntroNimbleFunctions.Rnw')}

\Sexpr{knit_child('chapter_QueryStructure.Rnw')}

\Sexpr{knit_child('chapter_ModelCalculations.Rnw')}

\Sexpr{knit_child('chapter_UserSamplers.Rnw')}
