...
does not treat the three parameters in a symmetric fashion but instead has a marginal prior on $\alpha_1$ that is standard normal and then constraints $\alpha_2$ to have a truncated normal distribution. This is not equivalent to a symmetric prior on the three $\alpha$s that puts 0 probability density on values that are not in order.
...
One should be cautious about constraints that would make it hard for an MCMC or optimization to move through the parameter space (such as equality constraints that involve two or more parameters). For such restrictive constraints, general purpose algorithms that are not tailored to the constraints are likely to fail. Also, one should of course always initialize a model such that the constraints are satisfied. Relying on NIMBLE's default of sampling from the prior does not ensure any constraints are satisfied. In many cases, parameterizing the model so that the constraints are automatically respected may be a better strategy than using \cd{dconstraint}.
