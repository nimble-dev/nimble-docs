...
The file \file{distributions\_inputList.R} in the R directory of the source
...
We also provide alternative names, which we call aliases (following JAGS), which can be used in the model definition in place of the canonical name. For a listing of these aliases see Table \ref{table:distributions-aliases}. This is so users can use the distribution names as used in other software, particularly the distribution names in R.
