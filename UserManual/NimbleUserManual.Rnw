...
pmyexp <- nimbleFunction(
    run = function(q = double(0), rate = double(0), lower_tail = integer(0), 
        log_p = integer(0)) {
        returnType(double(0))
        if(!lower_tail) {
            logp = -rate * q
...
qmyexp <- nimbleFunction(
    run = function(p = double(0), rate = double(0), lower_tail = integer(0), 
        log_p = integer(0)) {
        returnType(double(0))
        if(log_p) {
            p = exp(p)
...
\begin{itemize}
  ...
  \item All density functions must have as their last argument \cd{log\_value} and implement return of the log probability density. NIMBLE never passes \cd{log\_value = 1}, though of course we recommend you implement this simple calculation. Unfortunately, you cannot name this arguments \cd{log} as in R density functions as \cd{log} is used as the logarithm function in our generated C++ code.
  \item All distribution and quantile functions must have their last two arguments be (in order), \cd{lower\_tail} and \cd{log\_p}. These functions must be able to properly do their computation for \cd{lower\_tail = 1} (i.e., TRUE) and \cd{log\_p = 0} (i.e., FALSE), as these are the inputs we use when working with truncated distributions. It is your choice whether you implement the necessary calculations for other combinations of these inputs, but if you do not we recommend you print a warning \footnote{Unfortunately, we do not yet have error-handling in \cd{nimbleFunctions}.}. Unfortunately, you cannot name these arguments \cd{lower.tail} or \cd{log.p} as the use of a period conflicts with object-oriented code syntax when we create and compile C++ code.
\end{itemize}


Next we provide basic information on the structure of the distribution information passed to \cd{registerDistributions}. The single argument should be a list of lists, with each element a list providing information about a single user-defined distribution. Full details are given in the help information for \cd{registerDistributions}. Also note that additional examples can be seen in the \cd{R/distributions\_inputList.R} file for the default NIMBLE distributions in the source code for the NIMBLE package or on our Github repository. 
