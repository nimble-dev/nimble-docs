...
R be matched to the correct OS, i.e. R for snowleopard will attempt to use
...
by editing the Makevars file, see Writing R Externsions: 1.2.1. 
...
\item Allowance for constraints on the nodes in the model, using the \cd{dconstraint()} syntax, which shares some similarities with \cd{dinterval()}. See Section \ref{subsec:trunc}.
  \item Users can supply their own functions and distributions as \cd{nimbleFunctions} for use in BUGS code. See Section \ref{sec:user-defined}.
 \item Use of link functions in stochastic, as well as deterministic, declarations.\footnote{But beware of the possibility of needing to set values for nodes that NIMBLE creates behind the scenes.}
  \end{enumerate}
...
with BUGS and JAGS, we allow users to lump together data and constants
as the \cd{constants} argument to \cd{nimbleModel()}, in which case
...
their values by direct assignment.  \nm{constants} are hard-coded into
the model and are not variables in the model.

One can also provide variables appearing only on the right-hand side of
...
for the missing values. Of course in many contexts, one would want to 
...
Note that we currently have no automatic way of initializing the additional node(\cd{logit\_p} in this case), which is a parent node of the explicit node (\cd{p[i]}), without explicitly referring to the additional node by the name that NIMBLE generates. For deterministic declarations, this is of little import, but for stochastic declarations, it is a bit awkward. 
...
NIMBLE provides three ways to declare boundaries on the value of a
variable, each for different situations.  We introduce these and
comment on their relationships to JAGS' related features.

The three methods are:
\subsubsection{Truncation}
\cd{x $\sim$ dnorm(0, sd = 10) T(0, a)} or,  equivalently, \cd{x $\sim$ T(dnorm(0, sd = 10), 0, a)} declares that \cd{x} follows a normal distribution between 0 and \cd{a}.  Either boundary may be omitted or may be another node, such as \cd{a} in this example.  The first syntax follows JAGS, but in NIMBLE it can only be used when reading model code from a text file.  When writing model code in R, the second version must be used.  

Truncation means the probability density of \cd{x} is normalized.  In this example it would be the normal probability density divided by its integral from 0 to cd{a}\footnote{If you have a model object {model}, you can see exactly the calculation used by typing \cd{model\$nodes[['x']]\$calculate}}.  Like JAGS, NIMBLE also provides \cd{I} as a synonym for \cd{T} to accommodate older BUGS code, but \cd{T} is preferred because it disambiguates multiple usages of \cd{I} in WinBUGS.

Note that in NIMBLE, \cd{mu $\sim$ dfoo(theta) T(L, U)} restricts $X$ to lie in $(L,U]$ whereas in JAGS it restricts $X$ to lie in $[L,U]$.  This distinction makes no difference for continuous random variables.
% doesn't matter for continuous and we could follow jags but for discrete we would need to add extra code to deal with X == L -- thoughts?

NEED TO DISCUSS THIS.

\subsubsection{Censoring} Censoring refers to the situation where a datum represents a lower or upper bound on a value.  This is common in survival time studies, when we may know at least how long some patients lived (right-censoring) but not their ages of death, so their age of death is ``censored''.  NIMBLE adopts JAGS syntax for censoring, as follows (using right-censoring as an example):
 <<dinterval-example, eval=FALSE>>=
is.censored[i] ~ dinterval(t[i], c[i])
t[i] ~ dweib(r, mu[i])
@ 
where \cd{is.censored[i]} should be given as data with a value of 1 if \cd{t[i]} is right-censored (\cd{t[i] $\gt$ c[i]) and 0 if it is observed.  The data vector for \cd{t} should have \cd{NA} (indicating missing data) for any censored \cd{t[i]} entries. As a result, these nodes will be sampled in an MCMC.  The data vector for \cd{c} should give the censoring times (lower bound data in this case) corresponding to censored entries and \cd{0} (or anything less than the corresponding \cd{t[i]}) for uncensored entries.
  
Censoring differs from truncation because in censoring an observation gives an upper or lower bound on a random variable that could have taken any value, while in truncation an observation gives an observed value that, a priori, could not have occurred outside the truncation range.  In this example, the probability density for \cd{t[i]} is simply the Weibull. 

The \cd{dinterval} is not really a distribution but rather a trick: in the above example it gives a ``probability'' of 1 if \cd{t[i] $\gt$ c[i]} and 0 otherwise.  This means that \cd{t[i] $\le$ c[i]} is treated as impossible.  More generally, \cd{x[i] $\sim$ dinterval(t[i], c[i,])} is defined such that for a vector of increasing cutpoints, \cd{c[i,]}, \cd{t[i]} is enforced to fall within the cutpoint interval \cd{x[i]}.  This is done by setting data \cd{x[i]} as follows:
\begin{eqnarray}
x[i] = 0 & \mbox{if} & t[i] <= c[i, 1] \nonumber \\
x[i] = m & \mbox{if} & c[i, m] < t[i] <= c[i, m+1] \mbox{ for } 1 <= m <= M \nonumber \\
x[i] = M & \mbox{if} & c[i, M] < t[i].\nonumber
\end{eqnarray}
(The \cd{i} index is provided only for consistency with the previous example.)  The most common usage of \cd{dinterval} will be for left- and right-censored data, in which case $c[i,]$ will be a single value (and typically given as simply \cd{c[i]}), and for interval-censored data, in which case $c[i,]$ will be a vector of two values.  For left-censoring, one can use \cd{x[i] $\sim$ dinterval(t[i], c[i])} with \cd{x[i]} set to 0 or \cd{x[i] $\sim$ dinterval(c[i], t[i])} with \cd{x[i]} set to 1.
...
@ 
with data \cd{constraint.data} set to 1.  Formally, \cd{dconstraint(condition)} is a probability distribution on $\left{ 0, 1 \right}$ such that $P(1) = 1$ if \cd{constraint} is \cd{TRUE} and $P(0) = 1$ if \cd{constraint} is \cd{FALSE}. Like \cd{dinterval}, \cd{dconstraint} results in distributions that are not normalized, which makes most sense if the constraint is observed rather than established a priori.  Nodes following \cd{dconstraint} should be provided as data for reasons described above for \cd{dinterval}.
...
\textbf{Old draft below of ``Truncation, censoring and constraints'' below:}

NIMBLE follows JAGS in carefully distinguishing between truncation and censoring (see the JAGS manual for some discussion of this). In general, truncation and censoring are handled very similarly to JAGS, with limited exceptions noted below. In contrast we handle constraints very differently than JAGS.

\subsubsection{Truncation}

NIMBLE handles truncation similarly to JAGS. We use the \cd{T(,)} syntax, e.g.,
\cd{mu $\sim$ dnorm(1, sd = 10) T(0, )}. However, \cd{T(,)} only works when provided via BUGS code from an external file, because R cannot parse this syntax. As an alternative, we provide the following syntax, which is processed identically by NIMBLE: \cd{mu $\sim$ T(dnorm(1, sd = 10), 0, )}. As in JAGS, the truncation boundaries can be parameters or expressions, such as
\cd{mu $\sim$ T(dnorm(1, sd = 10), a, b + 3 )}, 
where \cd{a} and \cd{b} are also parameters in the model. Note that in NIMBLE, \cd{mu $\sim$ dfoo(theta) T(L, U)} restricts $X$ to lie in $(L,U]$ whereas in JAGS it restricts $X$ to lie in $[L,U]$.  This distinction makes no difference for continuous random variables.
% doesn't matter for continuous and we could follow jags but for discrete we would need to add extra code to deal with X == L -- thoughts?

In addition, as with JAGS, we allow the BUGS \cd{I(,)} syntax, which we treat as a synonym for \cd{T(,)}. As the JAGS manual notes, truncation and censoring are only equivalent in models in which the truncation or censoring applies to top-level nodes but we do allow use of \cd{I(,)} at other levels of the model, and this is still treated the same as  \cd{T(,)}. Note that unlike in BUGS, one cannot specify prior ordering using syntax such as
\begin{verbatim}
alpha[1] ~ dnorm(0, 1) I(, alpha[2])
alpha[2] ~ dnorm(0, 1) I(alpha[1], alpha[3])
alpha[3] ~ dnorm(0, 1) I(alpha[2], )
...
as this is does not represent a directed acyclic graph. Also, while prior ordering can be addressed in one fashion using \cd{T(,)}, this often will not produce the constraint the user actually wants. See our discussion of constraints below.

 

\subsubsection{Censoring}

We provide the \cd{dinterval} distribution to handle censoring, as in JAGS. For example, one can specify right-censored data, such as survival data as:
<<dinterval-example-old, eval=FALSE>>=
is.censored[i] ~ dinterval(t[i], c[i])
t[i] ~ dweib(r, mu[i])
@ 

where \cd{is.censored[i]} is an indicator variable that should be specified (as data) by the user to be 1 if $t_i$ is censored ($t_i > c_i$) and 0 if it is observed. If it is censored, then $t_i$ should be set to be NA in any vector, $t$, passed as input data. 

In general, \cd{x $\sim$ dinterval(t, c)} is defined such that for a vector of increasing cutpoints, $c$, the value of $x$ is the index of the interval in which $t$ falls:

\begin{eqnarray}
x = 0 & \mbox{if} & t <= c[1] \nonumber \\
x = m & \mbox{if} & c[m] < t <= c[m+1] \mbox{ for } 1 <= m <= M \nonumber \\
x = M & \mbox{if} & c[M] < t.\nonumber
\end{eqnarray}

The most common usage of \cd{dinterval} will be for left- and right-censored data, in which case $c$ will be a single value, and for interval-censored data, in which case $c$ will be a vector of two values. 

For nodes that are censored, NIMBLE will naturally treat these as unknown parameters. As a result, these nodes will be sampled in an MCMC.

In general, nodes that have the \cd{dinterval} distribution should be data nodes that are assigned a value. Otherwise, the node will be simulated into upon initialization and the resulting value will then constrain any nodes that depend on the node with the \cd{dinterval} distribution.  Also note that there is a distinction (discussed in the JAGS manual) between truncation applied to a node and censoring (in which the child of the node has a \cd{dinterval} distribution). Truncation applies as an a priori restriction while censoring is a posteriori (in particular conditional on the data). 

\subsubsection{Constraints and ordering}
\label{subsubsec:constraints}

NIMBLE also provides functionality for specifying constraints via the \cd{dconstraint} syntax. While this can be used for a variety of types of constraints, one specific usage is for specifying ordered parameters. In addition one can use \cd{dconstraint} to specify constraints on parameters more flexibly than using \cd{T(,)}. 

The basic syntax specifies a node to follow \cd{dconstraint(binaryValue)} where \cd{binaryValue} is an indicator variable whose value indicates whether a given condition is satisfied. For example, we could specify that the sum of two parameters must be positive using the last two lines of this BUGS code snippet:
<<dconstraint-example-old, eval=FALSE>>=
mu1 ~ dnorm(0, 1) 
mu2 ~ dnorm(0, 1) 
const ~ dconstraint( mu1 + mu2 > 0 )
@ 

One would then specifies \cd{const} to be data and to have value 1. This is equivalent to a half-normal distribution on the half-plane $\mu_1 + \mu_2 > 0$. However, note that this equivalence only holds when conditioning on \cd{const}. If one simulates from the model using the \cd{simulate} functions and the condition is not satisfied, then \cd{const} will be 0 and the log probability of \cd{const} (and therefore of the model as whole) will be $-\infty$.

\paragraph{Ordering}

To specify an ordering of parameters, such as $\alpha_1 <= \alpha_2 <= \alpha_3$ one can use \cd{dconstraint} as follows: 
<<ordering-example-old, eval=FALSE>>=
const ~ dconstraint( alpha1 <= alpha2 & alpha2 <= alpha3 )
@ 

As mentioned above, the constraint is an aspect of the data and will only be forced to be true when conditioning on the data.

NIMBLE does not support the JAGS \cd{dsort} syntax.

\paragraph{Cautions}

Note that if one uses the truncation syntax, \cd{T( , )} to try to specify a constraint, one will not specify the prior that one might think. 

For example, for prior ordering,
\begin{verbatim}
alpha1 ~ dnorm(0, 1)
alpha2 ~ dnorm(0, 1) T(alpha1, )
alpha3 ~ dnorm(0, 1) T(alpha2, )
...
does not treat the three parameters in a symmetric fashion but instead has a marginal prior on $\alpha_1$ that is standard normal and then constrains $\alpha_2$ to have a truncated normal distribution. This is not equivalent to a symmetric prior on the three $\alpha$s that assigns 0 probability density when values are not in order.
...
is not equivalent to the model specified above via \cd{dconstraint} and does not produce a bivariate half-normal distribution on the half-plane. 

One should be cautious about constraints that would make it hard for an MCMC or optimization to move through the parameter space (such as equality constraints that involve two or more parameters). For such restrictive constraints, general purpose algorithms that are not tailored to the constraints are likely to fail. Also, one should of course always initialize a model such that the constraints are satisfied. Relying on NIMBLE's default of sampling from the prior does not ensure any constraints are satisfied. 

In many cases, parameterizing the model so that the constraints are automatically respected may be a better strategy than using \cd{dconstraint}.
...
\section{Defining your own functions and distributions for use with BUGS code}
\label{sec:user-defined}

As of Version 0.4, NIMBLE now allows you to define your own functions and distributions (as \cd{nimbleFunctions} of course!) for use in BUGS code. As a result, NIMBLE frees you from being constrained to the functions and distributions listed in Section \ref{sec:more-details-nimble}.

Further, while NIMBLE at the moment does not allow the use of random indices, such as is common in clustering contexts, you may be able to analytically integrate over the random indices, resulting in a mixture distribution that you could implement as a user-defined distribution. This also would allow you to implement the \nm{dnormmix} distribution provided in JAGS.

This also allows you to specify models of reduced dimension, which can ease computations in various algorithms, such as MCMC. For example, one can write a Dirichlet-multinomial distribution that allows one to avoid working directly with the underlying latent probabilities in models with multinomial likelihoods and a Dirichlet prior on the multinomial probability vector.


\subsection{User-defined distributions}

To use a user-defined distribution, you need to, at a minimum, do the following:
\begin{enumerate}
...
Here is an extended example of providing a univariate distribution (simply duplicating the exponential distribution already provided by NIMBLE) and a multivariate distribution (the Dirichlet-multinomial, not provided by NIMBLE). Note that we can use truncation with the user-provided exponential distribution because we have provided distribution (``p'') and quantile (``q'') functions.
...
When you write your user-defined distribution-related functions, you should write them to take as input the parameters for a single parameterization, which will be the standard parameterization that NIMBLE will use. If you would like to allow for multiple parameterizations, you can do this via the \cd{Rdist} element of the list provided to \cd{registerDistributions} as discussed below.

If you provide CDF (distribution) (i.e., ``p'') and inverse CDF (quantile) (i.e., ``q'') functions, you can also make use of truncation with your distribution. In this case be sure to specify \cd{pqAvail = TRUE} when you call \cd{registerDistributions}.

Given the need to make distribution-related functions operate correctly in both R and C++, we have a few requirements for the syntax of distribution-related \cd{nimbleFunctions}:

\begin{itemize}
  \item Your distribution-related functions must have names that begin with ``d'', ``r'', ``p'' and ``q''. The name of the distribution must not be identical to any of the NIMBLE-provided distributions \footnote{If you wanted to create your own, say, \cd{dmnorm}, you just need to use a different name.}.
    \item Your functions must have the same distribution name as the \cd{BUGSdist} input to \cd{registerDistributions} (or as the \cd{Rdist} if you allow the BUGS distribution name to differ from the name as implemented in the \cd{nimbleFunctions}). 
  ...
  \item All density functions must have as their last argument \cd{log} and implement return of the log probability density. NIMBLE never makes use of \cd{log = 0}, though of course we recommend you implement this simple calculation. 
  \item All distribution and quantile functions must have their last two arguments be (in order), \cd{lower.tail} and \cd{log.p}. These functions must be able to properly do their computation for \cd{lower.tail = 1} (i.e., TRUE) and \cd{log.p = 0} (i.e., FALSE), as these are the inputs we use when working with truncated distributions. It is your choice whether you implement the necessary calculations for other combinations of these inputs, but if you do not we recommend you print a warning \footnote{Unfortunately, we do not yet have error-handling in \cd{nimbleFunctions}.}. 
\end{itemize}
...
To use a function you define in BUGS code, simply create a \cd{nimbleFunction} (without setup code) that implements the calculation. Then use it in your BUGS code. That's it.

One note of caution applies with regard to dimensionality. You can write \cd{nimbleFunctions} that take scalar input or higher-dimensional input and that produce either scalar or higher-dimensional return values. Make sure that the dimensionality specified in your \cd{nimbleFunction} matches how you use it in BUGS code. 

In general if you define  scalar parameters in your BUGS code you will want to define \cd{nimbleFunctions} that take scalar arguments, e.g.,
...
You can define vectorized functions and apply them to non-scalar variables, as shown here, regardless of whether the variable is a multivariate node or a collection of univariate nodes. 
...
code <- BUGScode({
    ...
    dmu[1:3] <- vdbl(mu[1:3])
    theta[1:3] ~ dmnorm(mu[1:3], prec[1:3, 1:3])
    dtheta[1:3] <- vdbl(theta[1:3])
})
@ 
                 
Note that there is no computational advantage to the use of \cd{vdbl} applied to \cd{mu} as opposed to applying \cd{dbl} to \cd{mu[i]} in a loop, because ultimately all the looping will be done in compiled code anyway. 

However, there is a subtle difference between the \cd{dmu} variables in the two examples.  In the first example, there are individual nodes for each \cd{dmu[i]}. In the second example, there is a single multivariate node, \cd{dmu}. Depending on the algorithms being applied to the model, either implementation may be more computationally efficient.

In addition to the general comment that you must respect the dimensions of arguments to \cd{nimbleFunctions}, note that you cannot apply a \cd{nimbleFunction} that takes as input non-scalar values and use a scalar variable as the input. I.e., in the example above you cannot have this syntax, \cd{dmu[i] <- vdbl(mu[i])}. Unlike in R, scalars are not simply vectors of length 1.
 
\subsection{Limitations}

As is generally the case for nimbleFunctions and BUGS models, you should create any user-defined functions or distributions in R's global environment (or use \cd{assign} to copy them there). NIMBLE does not have a scoping system set up, in part because we use Reference Classes to implement nimbleFunctions and R's Reference Classes don't use the usual scoping rules in R.

We do not yet allow users to define their own link functions, but we plan to implement this soon.
...
all generated randomly\footnote{Of course in general, explicitly calculating the inverse is not the recommended numerical recipe for least squares.}.  Such functions can be called from other nimbleFunctions.
...
\subsection{Naming arguments and variables}
\label{sec:naming-in-DSL}

Because the names of arguments to nimbleFunctions and those of variables in \cd{run} code are 
used directly in C++, one cannot use a period in the names of arguments or run-time variables because C++ interprets the period as the dot operator for class membership access.

Similarly, do not name an argument or run-time variable with the same name as a function that
is used in the \cd{run} code, as this will cause a conflict in the C++ code.
...
Distribution ``d'', ``r'', ``p'', and ``q'' functions can all be used from
nimbleFunctions, but the care is needed with the order of arguments.
For distributions available in R, nimbleFunction parameterizations
match those of R, \textit{which may differ in default order
  from the same-named BUGS distributions}.  For distributions
available in BUGS but not R, nimbleFunctions currently support the default BUGS
parameterization but not alternatives.

Supported distributions include (THIS NEEDS REVIEW):
\begin{itemize}
...
\item Similarly, \cd{getLogProb} should only be used when one is sure
  ...
  log probabilities. 
\item We have tried to make NIMBLE's handling of multivariate objects
  flexible, but how you choose to set things up could affect
  computational efficiency.  We'll explore that more in the future.
\item Any nimbleFunctions or BUGS models should be created in R's global environment. Otherwise NIMBLE will have trouble finding such objects and using them with each other during compilation and running of nimbleFunctions. This is in part a result of how R's Reference Classes do not use the usual R scoping rules.
\end{itemize}
