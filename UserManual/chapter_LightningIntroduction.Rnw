...
We'll use the \nm{pump} model example from BUGS\footnote{The data set
  describes failure times of some pumps.}.  We could load the model
from the standard BUGS example file formats (\ref{sec:readBUGSmodel}), but instead we'll show
...
parent nodes of all \cd{theta[i]} nodes were fully initialized, including any non-stochastic nodes such
...
The call to \cd{calculate(pump, `theta')}
...
\cd{alpha}\footnote{We haven't set up conjugate relationships for an
  exponential yet.}, on which the default sampler is a random walk Metropolis sampler.
...
al. (1993)\footnote{George, E.I., Makov, U.E. \& Smith,
  ...
  Their numbers were accidentally swapped in Table 2.}.  Some
discrepancy is to be expected since it is a Monte Carlo algorithm.
...
\item The \cd{setup} code creates a \cd{modelValues} object to hold multiple sets of
...
\item Multiple instances, or ``specializations'', can be made by
  ...
  \cd{setup} code and a \cd{run} method (member function).  Arbitrary
  other methods can be provided with a \cd{methods} argument,
  following the syntax of R's \cd{setRefClass} function.
\end{enumerate}
