...
\item The special function \cd{copy} is used here to record values
  from the model into the \cd{modelValues} object.  
\item Multiple instances, or ``specializations'', can be made by
  ...
  % TODO: I think this last sentence is too involved for the lightning intro - CJP
\end{enumerate}
...
seed, we got identical results for \cd{theta[1:5]} and their dependencies, but it happens much faster.
