...
<<exampleDefineNimList2>>=
 nimbleListTypes <- list(nimbleType(name = 'nimListInteger', type = 'integer', dim = 0),
                         nimbleType(name = 'nimListMatrix', type = 'double', dim = 2))
...
<<exampleCreateNimList>>=
 exampleNimList <- exampleNimListDef$new(nimListInteger = 1, nimListMatrix = diag(2))
@
...
\subsection{Using \cd{eigen()} and \cd{svd()} in \cd{nimbleFunction}s} \label{sec:eigen-nimFunctions}
