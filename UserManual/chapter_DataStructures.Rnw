...
{\color{red} as with calcNodes, I'd like to discuss when a user would be expected to use this and figure out if this is where to talk about it in light of that}
...
<<exampleDefineNimList2>>=
 nimbleListTypes <- list(nimbleType(name = 'nimListInteger',
                                    type = 'integer', dim = 0),
                         nimbleType(name = 'nimListMatrix',
                                    type = 'double', dim = 2))
...
<<exampleCreateNimList>>=
 exampleNimList <- exampleNimListDef$new(nimListInteger = 1,
                                         nimListMatrix = diag(2))
@
Once created, \cd{nimbleList} elements can be accessed using the \cd{\$} operator, just as with \cd{list}s in \cd{R}.  For example, the value of the \cd{nimListInteger} element of our \cd{exampleNimbleList} can be set to \cd{11} using
<<exampleSetNimList>>=
 exampleNimList$nimListInteger <- 11
@
...
Below, we demonstrate a function that takes the \cd{exampleNimList} as an argument, modifies its \cd{nimListMatrix} element, and returns the \cd{nimbleList}.  We note that \cd{nimbleFunction}s that take \cd{nimbleList}s as run arguments must have setup code.  For functions that need no setup code, setting \cd{setup = TRUE} will allow these functions to work correctly.
...
nimListFunctionGenerator <- nimbleFunction(
  ...
  run = function(argumentList = exampleNimListDef()){
    ...
    argumentList$nimListMatrix  <- onesMatrix
...
\cd{nimbleList} arguments to run functions are passed by reference -- this means that if an element of a \cd{nimbleList} argument is modified within a function, that element will remain modified when the function has finished running.  To see this, we can inspect the value of the \cd{nimListMatrix} element of the \cd{exampleNimList} object:

<<exampleNimListReference>>=
exampleNimList$nimListMatrix
@
Note that the \cd{nimListMatrix} was assigned a new value (a matrix of all 1's) within the function, and this new value persists outside of the function.

In addition to storing basic data types, \cd{nimbleList}s can also store other \cd{nimbleList}s.  To achieve this, we must create a \cd{nimbleList} definition that declares the types of nested \cd{nimbleList}s a \cd{nimbleList} will store.  Below, we create two types of \cd{nimbleList}s: the first, named \cd{innerNimList}, will be stored inside the second, named \cd{outerNimList}:

<<exampleNestedNimList>>=
## first, create definitions for both inner and outer nimbleLists
innerNimListDef <- nimbleList(nimListCharacter = character(0))
outerNimListDef <- nimbleList(innerNimList = innerNimListDef(),
                                nimListDouble = double(0))

## then, create outer nimbleList
outerNimList <- outerNimListDef$new(nimListDouble = 3.14)

## access element of inner nimbleList
outerNimList$innerNimList$nimListCharacter <- "hello, world"
@
