...
\item If the node is vectorized, \emph{i.e.,} was specified using a multivariate distribution, then a single \nm{RW_block} sampler is assigned to jointly sample all scalar components of the multivariate node.  This sampler performs multi-dimensional Metropolis-Hastings random walk sampling.
