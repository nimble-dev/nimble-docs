...
The default specification may be replaced by that generated from an automated parameter blocking algorithm.  The purpose of this algorithm is to determine groupings of model nodes which can be blocked together (jointly sampled) to increase overall MCMC efficiency.  Overall efficiency is defined as the effective sample size of the slowest-mixing model node divided by algorithm runtime.  An MCMC specification using the automated blocking sampler assignments can be created using:
...
The \cd{nodes} argument may also be provided with the value \cd{NULL}, \cd{character()}, or \cd{list()}, any of which will result in an MCMC specification containing no samplers.
...
Once an \cd{MCMCspec} object has been created, additional samplers may be added to the specification, by calling the \cd{addSampler} method on the \cd{MCMCspec} object.

The frst argument to \cd{addSampler}, which is compulsory, is the \cd{target} argument.   This must be a character string (or vector) specifying one or more model nodes or variables.  The sampler function will operate on (sample) this ``target" node or set of model nodes.
 
The second argument to \cd{addSampler} is the sampler \cd{type}.  This may be a character string, specifying the name of the sampler to be added.  For example, \cd{type = `RW'} specifies the Metropolis-Hastings random walk sampler, and \cd{type = `slice'} specifies a slice sampler.  Alternatively, the \cd{type} argument may also be provided with a (non-specialized) nimbleFunction object.  This function object must be a valid sampling algorithm, which has been generated as the result of \cd{nimbleFunction}.  In this usage, the \cd{name} argument may be provided to \cd{addSampler}, to specify a character string name for the sampler being added.  This may be useful for the programmatic generation of MCMC algorithms.  The default value for the \cd{type} argument is \cd{``RW''} which specifies adaptive scalar random walk (Metropolis-Hastings) sampling.  It is worth nothing that this default value will result in an error if the \cd{target} argument specifies more than one target node.
...
The current, ordered, list of all samplers in the MCMC specification may be printed by calling the \cd{getSamplers} method. Each sampler is displayed, along with the value of all control parameters, and the index corresponding to its position in the sampler ordering.  These indices are useful for removing and re-ordering the samplers.  \cd{getSamplers} may also be provided a character vector argument, in which case it will only print those samplers acting on any component of the model nodes or variables specified in this argument.

The existing samplers may be re-ordered using the \cd{setSamplers} method. The \cd{ind} argument is a vector of sampler indices, or a character vector of model node or variable names.  The samplers in the MCMC specification will be replaced by the samplers corresponding to the indices provided, or those samplers acting on the target nodes specified.  A few examples of how this may operate (all examples assume the \cd{MCMCspec} object initially contains 10 samplers; each example call is independent of the others) are as follows:
...
\item[\cd{`nimble'}] NIMBLE MCMC using the default specification
\item[\cd{`nimble\_RW'}] NIMBLE MCMC using the default specification with \cd{onlyRW = TRUE}
\item[\cd{`nimble\_slice'}] NIMBLE MCMC using the default specification with \cd{onlySlice= TRUE}
...
## adding it to MCMC specifications, will be 'my_RW'
