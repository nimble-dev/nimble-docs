...
The mandatory \cd{type} argument to \cd{addSampler()} may be a character string, specifying the name of the sampler to be added.  For example, \cd{type = `RW'} specifies the Metropolis-Hastings random walk sampler, and \cd{type = `slice'} specifies a slice sampler.  Alternatively, the \cd{type} argument may also be provided with a (non-specialized) nimbleFunction object.  This function object must be a valid sampling algorithm, which has been generated as the result of \cd{nimbleFunction()}.  In this usage, the \cd{name} argument may be provided to \cd{addSampler()}, to specify a character string name for the sampler being added.  This may be useful for the programmatic generation of MCMC algorithms.

The mandatory \cd{target} argument to \cd{addSampler()} is a character string (or vector) specifying one or more model nodes.  The sampler function will operate on (sample) this ``target" node (or nodes).  For example, a \cd{slice} sampler can only sample a single \cd{target} node, but the RW block sampler (\cd{RW\_block}) can sample multiple model nodes. 
...
mcmcspec$addSampler(type = 'end', target = 'y[1]')
@
...
mcmcspec$addSampler(type = 'RW', target = 'a',
     control = list(adaptive = FALSE, scale = 3))

mcmcspec$addSampler(type = 'RW', target = 'b',
...
mcmcspec$addSampler(type = 'RW_block', target = c('a', 'b', 'c'))
@
...
mcmcspec$addSampler(type = 'slice', target = 'y[1]',
     control = list(adaptive = FALSE, sliceWidth = 3))

mcmcspec$addSampler(type = 'slice', target = 'y[2]', 
...
mcmcspec$addSampler(type = 'crossLevel', target = c('a', 'b', 'c'))
@
...
Example usage:
 <<RW_ll, eval=FALSE>>=
 mcmcspec$addSampler(type = 'RW_llFunction', target = 'p',
...
mcmcspec_CL$addSampler(type = 'crossLevel', target = c('a[1]', 'b[1]'))

mcmcspec_CL$addSampler(type = 'crossLevel', target = c('a[2]', 'b[2]'))
...
output <- MCMCsuite(
    ...
    MCMCdefs = list(
        nimble_CL = quote({
            ...
            mcmcspec$addSampler(type = 'crossLevel', target = c('a[1]', 'b[1]'))
            mcmcspec$addSampler(type = 'crossLevel', target = c('a[2]', 'b[2]'))
            mcmcspec
        })),
...
mcmcspec$addSampler(type = 'my_RW',
     target = 'x', control = list(scale = 0.1))
