...
the most part, if you have BUGS code, you can try NIMBLE.
  ...
  {\color{red} Not sure we've generally used the term 'C-model' --
    these only appear in this Chapter: I
    think 'R model' and 'compiled model' is our standard phrasing;
    thoughts on whether to reword the above?}
...
\item NIMBLE generates C++ code for the run-time function(s), compiles
  ...
  C-nimbleFunctions, respectively.   In Version \ver, the behavior
...
{\color{red} Actually we never use {R,C}-nimbleFunction elsewhere - I
  vote for taking this out - CJP}
...
\item A \cd{nimbleFunction} that provides ability to simulate,
 ...
 sets of values for
  arbitrary sets of nodes. {\color{red} CJP would like to remove this bullet
    as the nimbleFunctions referred to are really a programming tool and not algorithms}
...
{\color{red} CJP - so is the only difference between calculate and
  ...
  calcNodes etc. include operations on dependencies? If so, I
  propose we extend calculate, simulate, getLogProb to have an
  argument includeDependencies with default FALSE}
