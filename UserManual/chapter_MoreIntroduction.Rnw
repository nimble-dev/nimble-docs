...
\item MCMC with samplers including conjugate, slice, adaptive random
  ...
  model object and creates an MCMC configuration object representing
  choices of which kind of sampler to use for each node.  This MCMC
  configuration can be modified in R, such as adding new samplers for
...
\item A \cd{nimbleFunction} that provides a basic particle filter for
  a state-space model.
