...
(but don't have to) use BUGS models.  The main ways they can use
BUGS models are:
...
\item A \nm{setup} function within a nimbleFunction gives the steps
  ...
  of a model will need to be calculated for a MCMC sampler. \nm{setup}
...
\item One or more \nm{run} functions within a nimbleFunction give
  ...
  \nm{setup} function, such as the iterations of a MCMC sampler.
  Formally, \nm{run} code is written in the NIMBLE language, which you
...
\item When you are ready, NIMBLE can generate C++ code, compile it,
  ...
  C++ internally.  Again, we refer to these steps all together as ``compilation.''  The behavior of compiled \cd{nimbleFunction}s is
...
a nimbleFunction as a class definition. The \nm{setup} function
initializes a new object and \nm{run} functions are class methods.
Member data are determined automatically as the objects from a
\nm{setup} function needed in \nm{run} functions.  If no \cd{setup}
...
\item MCMC with samplers including conjugate (Gibbs), slice, adaptive
  ...
  scale), adaptive block random walk, and elliptical slice. You can
...
\item An ascent-based Monte Carlo Expectation Maximization (MCEM)
  algorithm.
\item A variety of basic algorithms that can be used as programming
  ...
  \item Other functions described in Chapter \ref{cha:algos-provided}.
  \end{enumerate}

\end{enumerate}

{\color{red} Add references where appropriate.}.
 
{\color{red} CJP - so is the only difference between calculate and
  calcNodes (and similarly for simNodes, getLogProbNodes) that
  calcNodes etc. include operations on dependencies? If so, I propose
  we extend calculate, simulate, getLogProb to have an argument
  includeDependencies with default FALSE.} {\color{blue} PdV - There
  were a couple of things conflated here, but the question is still
  valid.  The suggestion seems like a slippery slope towards trying to
  make calculate (etc.) smarter, which could get away from the core
  design ideas.}
