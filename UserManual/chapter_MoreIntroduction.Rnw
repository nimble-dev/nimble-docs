...
to use MCMCs run in BUGS, JAGS, or other systems in combination with NIMBLE's other algorithms,
you can\footnote{And you will be able to do so more thoroughly in the future.}.  We adopted BUGS
...
the OpenBUGS or JAGS websites.  For the most part, if you have BUGS code, you can try
NIMBLE.
