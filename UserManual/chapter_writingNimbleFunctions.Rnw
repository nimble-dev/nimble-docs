...
Use of \cd{numeric()}, \cd{integer()}, \cd{matrix()} or \cd{array()} to create 
non-scalar local variables (see \ref{sec:decl-local-vars}).  Scalar-valued local 
variables need not be declared.

\subsubsection{Mathematical operations}
Numeric scalar or matrix mathematical operations listed in tables
\ref{table:functions} and \ref{table:functions-matrix}.  Also see \ref{sec:basic-math-linear}.
...
or any user-defined distributions (see \ref{sec:user-defined}). 

\subsubsection{Control flow}
Standard control-flow commands including \cd{if()}, \cd{for()},
\cd{while()}, and \cd{return()} (see \ref{sec:basic-flow-control}).
...
returned (included as part of the function body) (see \ref{sec:decl-argum-types}). 
...
\ref{sec:getting-setting-more}), and \cd{getParam()} (see \ref{sec:getParam}). 

Functions for interacting with \cd{modelValues} objects (see \ref{sec:modelValues-struct}).
...
Calls to other (nested) \cd{nimbleFunction}s (see
\ref{sec:using-other-nimbl}), calls to \cd{nimbleFunctions} in
nimbleFunctionLists (see \ref{sec:virt-nimbl-nimbl}), 
and calls to other member methods of the same 
\cd{nimbleFunction} (see \ref{sec:incl-other-meth}). 
