...
Local variables are generally created automatically in NIMBLE functions, and
their types will be automatically inferred.  For
example, \cd{x <- A \%*\% B} will create \cd{x} as a matrix.  However,
if a variable is constructed only through indexed assignment, then it must be
explicitly created first using one of the variable creation functions.
For example, in the following code \cd{x} must be created first:
...
Accordingly, scalar variables \emph{never} need to be be
created in advance.  Since as they are not indexed, they are always
created in a single (non-indexed) assignment statement.

NIMBLE provides several functions for creating non-scalar local
variables.  These functions are very similar to those of R,
except with additional arguments that provide additional
functionality.  \cd{numeric()} and \cd{integer()} create
floating-point and integer vectors (1-dimensional objects).
\cd{matrix()} creates 2-dimensional matrix
objects, and \cd{array()} can create 1-dimensional vectors,
2-dimensional matrices, or arrays of higher dimension.

Note that all arguments to these functions can be
arbitrary DSL expressions (using other variables, or mathematical
expressions, for example), which evaluate to appropriate values for
each argument.

After creating a non-scalar variable using one of these functions, or
through direct (non-indexed) assignment into the variable name, the
\cd{setSize()} method may still be used to resize the variable, noting
that the dimension of the variable must remain the same (that is, a
1-dimensional vector must remain as a vector, and a 2-dimensional
matrix must remain as a matrix).  Note also that calls to \cd{setSize()}
do \emph{not} necessarily preserve the contents of the object.
...
\cd{numeric()} creates a 1-dimensional vector object of floating-point 
(non-integer) values.  The \cd{length} argument specifies the vector 
length (default 0), and the \cd{value} argument specifies the initial 
value for all vector elements (default 0). The \cd{init} argument specifies 
whether or not to initialize the elements (default \cd{TRUE}).

<<exampleNumeric, eval=FALSE>>=
## creates a length 10 vector, all elements initialized to 2 
...
\cd{integer()} creates a 1-dimensional vector object of integer 
values.  The \cd{length} argument specifies the vector 
length (default 0), and the \cd{value} argument specifies the initial 
value for all vector elements (default 0). The \cd{init} argument specifies 
whether or not to initialize the elements (default \cd{TRUE}).
...
\cd{matrix()} creates a 2-dimensional matrix object.  The resulting matrix can 
contain floating-point numbers (using the default argument valule of 
\cd{type = 'double'}), or integer values by specifying the argument 
\cd{type = 'integer'}. 
The \cd{nrow} and \cd{ncol} arguments specifiy the size of this 
matrix, being the number of rows and number of columns, respectively. 
The \cd{value} argument specifies the initial 
value for all matrix elements (default 0). The \cd{init} argument specifies 
whether or not to initialize the elements (default \cd{TRUE}).

<<exampleMatrix, eval=FALSE>>=
## creates a 10-by-1 column matrix of 1's
...
\cd{array()} creates a vector or higher-dimensional object, depending on 
the \cd{dim} argument which specifies the dimensionality.  Similiar to \cd{matrix()},
\cd{array()} can create objects with floating-point numbers or 
intergers, depending on the \cd{type} argument.  The \cd{value} argument specifies the initial 
value for all array elements (default 0). The \cd{init} argument specifies 
whether or not to initialize the elements (default \cd{TRUE}).
