...
or any user-defined distributions (Section \ref{sec:user-defined}). 
...
\ref{sec:getting-setting-more}), and \cd{getParam()} (Section \ref{sec:getParam}). 
...
The resulting matrix may have it's elements modified, or be used in
