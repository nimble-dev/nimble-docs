...
is needed \footnote{Skipping initialization is more efficient, but
  this will typically only be noticeable only for functions called many,
...
The \cd{nrow} and \cd{ncol} arguments specifiy the number of rows and columns, respectively. 
