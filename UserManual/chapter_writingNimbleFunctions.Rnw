...
\chapter{Writing \cd{nimbleFunction}s  that interact with model}
...
\section{What's allowed in the NIMBLE language}
\label{sec:nimble-dsl}

{\color{red} [refer back to chapter on RC functions]}
...
\subsubsection{Operations on \cd{model} and \cd{modelValues} objects}
Functions for interacting with NIMBLE \cd{model}
objects, including \cd{simulate()}, \cd{calculate()}, \cd{calculateDiff()},
\cd{getLogProb()} (see \ref{sec:cdcalc-cdsim-cdgetl}), \cd{copy()} (see 
\ref{sec:access-model-modelv}), \cd{values()} (see 
\ref{sec:getting-setting-more}), \cd{getParam()} (Section \ref{sec:getParam}), and \cd{getBound()} (Section \ref{sec:getBound}). 

Functions for interacting with \cd{modelValues} objects (Section \ref{sec:modelValues-struct}).

\cd{model} or \cd{modelValues} objects must originate from 
\cd{setup} function arguments or a declaration within the \cd{setup} function. 

\subsubsection{Other nimbleFunctions}
Calls to other (nested) \cd{nimbleFunction}s (Section
\ref{sec:using-other-nimbl}), calls to \cd{nimbleFunctions} in
nimbleFunctionLists (Section \ref{sec:virt-nimbl-nimbl}), 
and calls to other member methods of the same 
\cd{nimbleFunction} (Section \ref{sec:incl-other-meth}). 

Nested \cd{nimbleFunctions} or \cd{nimbleFunctionLists} must originate from 
\cd{setup} function arguments or a declaration within the \cd{setup} function. 
...
In addition to the language components used in simple nimbleFunction (Section \ref{sec:nimble-lang-comp-simple}), the NIMBLE language contains a variety of functionality for interacting with models. 
...
\section{Writing user-defined samplers to extend NIMBLE's MCMC engine}
...
{\color{red} We may want to move the comments on the requirements into standard text; also don't samplers need to be named 'sampler_{name}'? More extended user-defined samplers are in the user_sampler module of our outreach; should we put more of that here?}
