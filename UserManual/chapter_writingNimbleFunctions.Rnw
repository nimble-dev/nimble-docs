...
\item BUGS model nodes are implemented as nimbleFunctions with member
  ...
  1/scale) and a more generic way to do so using the function
  \cd{getParam}.  
...
represents the creation of \cd{x} by construction.  This works in R,
but not in compiled nimbleFunctions.  For a compiled nimbleFunction,
prior to the above block of code, one
would need to declare \cd{x} by either 
...
sizes to 10 and 5, respectively.  In either case the sizes can be
