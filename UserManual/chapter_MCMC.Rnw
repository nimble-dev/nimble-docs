...
The current release of NIMBLE supports conjugate sampling of the conjugate relationships listed in Table~\ref{table:conjugaciesSupported}.
...
\begin{itemize}
\item MCMC algorithms to execute, optionally including WinBUGS, JAGS, Stan, and various flavours of NIMBLE's MCMC
...
The default value for the \cd{MCMCs} argument is \cd{c(`jags',`nimble', `nimble\_RW', `nimble\_slice', `autoBlock')}.  
