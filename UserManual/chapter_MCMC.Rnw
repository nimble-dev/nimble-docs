...
Configuring, building and running and MCMC algorithm for a NIMBLE model takes the following steps:

\begin{enumerate}
\item Optionally configure and customize an MCMC for a particular model:
  \begin{enumerate}
  \item Use \cd{configureMCMC} to create an MCMC configuration (see Chapters \ref{cha:writing-models}-\ref{cha:building-models}).  The configuration is a list of samplers with the node(s) they will sample.
  \item Modify the MCMC configuration to 
    \begin{enumerate}
    \item add, remove, or re-order the list of samplers (section \ref{sec:samplers-provided}, \cd{help(samplers)} in R for details), including adding your own samplers (see Chapter \ref{});
    \item change the tuning parameters or adaptive properties of individual samplers;
    \item change the variables to monitor (record for output) and thinning intervals for sample collection.
    \end{enumerate}
...
In addition NIMBLE provides several functions to simplify running one or multiple MCMCs:
\begin{itemize}
\item \cd{runMCMC} simplifies the steps of modifying initial values, removing burn-in samples, returning samples in the form of a \cd{coda} \cd{mcmc} objec, and running multiple chains for the same MCMC (section \ref{sec:runMCMC}).
\item \cd{MCMCsuite} can run multiple, different MCMCs for the same model.  These can include external MCMCs such as from WinBUGS, OpenBUGS, JAGS or Stan (section \ref{mcmc-suite-compare-mcmcs}).
...
\begin{enumerate}
\item If the node has no stochastic dependents, a predictive \cd{posterior\_predictive} sampler is assigned.  The \cd{posterior\_predictive} sampling algorithm merely calls \cd{simulate} for the particular node.
...
\item If the node is binary-valued (strictly taking values 0 or 1), then a \cd{binary} sampler is assigned.  This sampler calculates the conditional probability for both possible node values and draws the new node value from the distribution, in effect making a Gibbs sampler. 
...
\item If a node follows a Dirichlet distribution, then the \cd{RW\_dirichlet} sampler is assigned.  This is a random walk sampler in the space of the simplex defined by the Dirichlet.
...
\item those supplied in \cd{nimbleOptions(MCMCcontrolDefaultList)}.
\end{enumerate}

A call to \cd{addSampler} results in a single instance of the specified sampler, which will be specialized to the specified \cd{target} model node or nodes, being added at end of the current sampler list.  Adding a sampler for a node will \emph{not} automatically remove any existing samplers on that node. 

\subsubsection{Printing, re-ordering, modifying and removing samplers: \cd{printSamplers}, \cd{getSamplers}, \cd{getSamplerDefinition}, \cd{setSamplers}, \cd{removeSamplers}, \cd{setTarget}, and \cd{setControl}}

{\color{red} PdV suggests splitting this into 2 or 3 subsubsections.}
...
\subsubsection{Monitors and thinning intervals: \cd{getMonitors}, \cd{addMonitors}, \cd{setThin}, and \cd{resetMonitors}}
