...
%% > library(knitr);   try(knit2pdf('includesWritingNFsonly.Rnw'), silent=TRUE);   system('open includesWritingNFsonly.pdf')
...
<<eval=FALSE>>=
mcmcConf <- configureMCMC(Rmodel)
mcmcConf$addMonitors(c('logProb_alpha', 'logProb_sigma_mu', 'logProb_Y'))
Rmcmc <- buildMCMC(mcmcConf)
