...
\begin{enumerate}
\item Optionally create and customize an MCMC configuration for a particular model:
  \begin{enumerate}
  \item Use \cd{configureMCMC} to create an MCMC configuration (see Chapters \ref{cha:writing-models}-\ref{cha:building-models}).  The configuration contains a list of samplers with the node(s) they will sample.
  \item Customize the MCMC configuration: 
    \begin{enumerate}
    \item add, remove, or re-order the list of samplers (section \ref{sec:samplers-provided} and \cd{help(samplers)} in R for details), including adding your own samplers (section \ref{});
  ...
  \end{enumerate}
\item Use \cd{buildMCMC} to build the MCMC object and its samplers either from the model (using default MCMC configuration) or from a customized MCMC configuration.
...
End-to-end examples of MCMC in NIMBLE have been shown in sections \ref{}, \ref{} and \ref{}.
...
Details of each sampler and its control parameters can be found by \cd{help(samplers)} in R with \cd{nimble} loaded.

\subsubsection{Modifying the default sampler assignments}

\cd{configureMCMC} can take control arguments \cd{useConjugacy}, \cd{onlyRW}, \cd{onlySlice}, and \\
\cd{multivariateNodesAsScalars} to modify default sampler assignments.
These tell \cd{configureMCMC} whether to assign conjugate sampler
when possible, whether to use only random walk or only slice samplers,
and whether to sample scalar elements of multivariate nodes
individually, respectively.  See \cd{help(configureMCMC)} for usage
details.
...
The default MCMC configuration includes monitors on all top-level stochastic nodes of the model.  MCMC configurations include two sets of monitors, each with different thinning intervals.  By default, the second set of monitors (\cd{monitors2}) is empty. 
...
If you plan to customize the choice of all samplers, it can be useful to obtain a configuration with no sampler assignments at all.  This can be done by \cd{nodes = NULL}, \cd{nodes = character()}, or \cd{nodes = list()}. 
...
Samplers may be added to a configuration using the \cd{addSampler} method of the \cd{MCMCconf} object.  The frst argument gives the node(s) to be sampled, called the \cd{target}, as a character vector.   The second argument gives the type of sampler, which may be provided as a character string or a \cd{nimbleFunction} object. Valid character strings include `posterior\_predictive', `RW', `RW\_block', `slice', `RW\_multinomial', `RW\_dirichlet', `binary', `ess', `crossLevel', and `RW\_llFunction'.  Added samplers can be labeled with a \cd{name} argument, which is used in output of \cd{printSamplers}.


{\color{red} Question from PdV: Isn't `conjugate' now a valid sampler type?}

Writing a new sampler as a \cd{nimbleFunction} is covered in section \ref{}.
...
\item those supplied in the \cd{control} list argument to \cd{configureMCMC};
\item the defaults, which are in \cd{nimbleOptions(`MCMCcontrolDefaultList')}.
\end{enumerate}
...
\subsubsection{Measuring sampler computation times: \cd{getTimes}} \label{sec:sampler-time}
...
The \cd{run} method has an optional \cd{time} argument.  When \cd{time = TRUE}, the net computation time spent in each sampler will be recorded.  See Section \ref{sec:sampler-time}.
...
Obtaining samples as matrices is most common, but see section \ref{sec:modelValues-struct} for more about programming with \cd{modelValues} objects, especially if you want to write nimbleFunctions to use the samples.

\section{Running multiple MCMC chains}
\label{sec:runMCMC}

Once an MCMC algorithm has been created using \cd{buildMCMC}, the function \cd{runMCMC} can be used to run multiple chains and extract samples (see \cd{help(runMCMC)} in R).   \cd{runMCMC} can be used to execute compiled or uncompiled algorithms, although uncompiled algorithms will be much slower.  Specifically, \cd{runMCMC} takes arguments to control:
...
Most documentation of MCMC samplers provided with NIMBLE can be found by \cd{help(samplers)} in R.  Here we provide additional explanation of conjugate samplers and how complete customization can be achieved by making a sampler use an arbitrary log likelihood function, such as to build a particle MCMC algorithm.

{\color{red} PdV suggests cutting most of this since it is a copy of what is in the Rd pages.  The example lines could be moved to the roxygen entries.}
...
By default, \cd{configureMCMC()} and \cd{buildMCMC()} will assign conjugate samplers to all nodes satisfying a conjugate relationship, unless the option \cd{useConjugacy = FALSE} is specified.  %The conjugate relationships supported are defined in the system-level variable\\ \cd{nimble:::conjugacyRelationshipsInputList}.  %Subsequently, conjugate samplers can be added to particular nodes in an MCMC configuration object (\cd{conf}) using \cd{conf\$addSampler(target = 'x', type = 'conjugate')}, to add a conjugate sampler for model node \cd{'x'}.  If the specified node does not satisfy any conjugate relationship, then an error occurs and no sampler is added to the configuration object.

The current release of NIMBLE supports conjugate sampling of the relationships listed in Table~\ref{table:conjugaciesSupported}\footnote{NIMBLE's internal definitions of these relationships can be viewed by \cd{nimble:::conjugacyRelationshipsInputList}.}.

\input{conjugacyTable.tex}

Conjugate sampler functions may (optionally) dynamically check that their own posterior likelihood calculations are correct.  If incorrect, a warning is issued.  However, this functionality will roughly double the run-time required for conjugate sampling.  By default, this option is disabled in NIMBLE.  This option may be enabled by \cd{nimbleOptions(verifyConjugatePosteriors = TRUE)}.

If one wants information about conjugate node relationships for other purposes, they can be obtained using \cd{model\$checkConjugacy()}.  This returns a named list describing all conjugate nodes.  The \cd{checkConjugacy()} method can also accept a character vector argument specifying a subset of node names to check for conjugacy. 
...
% Example usage:
% <<dirichlet-sampler, eval=FALSE>>=
% mcmcConf$addSampler(target = 'p[1:5]', type = 'RW_dirichlet')
% @
