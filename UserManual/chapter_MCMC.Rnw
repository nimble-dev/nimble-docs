...
mcmcspec$addSampler(target = 'x[1:10]', type = 'ess')
@
...
For state space models, a particle MCMC sampler can be specified for top-level parameters.  This sampler is described in greater detail in \ref{sec:particle-mcmc}
...
In addition to these per-node measures of efficiency, an additional return list element is also provided.  This element, \nm{efficiency}, is itself a named list containing two elements: \nm{min} and \nm{mean}, which contain the minimal and mean efficiencies (effective sample size per second of algorithm runtime) across all monitored nodes for each algorithm.
