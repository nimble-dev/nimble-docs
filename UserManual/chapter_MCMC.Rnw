...
\item If a node follows a Dirichlet distribution and there is no conjugate relationship with the distributions of its stochastic dependents, then the \cd{RW\_dirichlet} sampler is assigned.
