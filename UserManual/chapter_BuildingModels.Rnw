...
``descendants'') in a model using \cd{getDependencies}. There are a variety of arguments to \cd{getDependencies} that
...
default \cd{getDependencies} returns descendants up through the next
