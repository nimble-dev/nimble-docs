...
Models in NIMBLE are specified using a variation on the BUGS language, a language also used in the various BUGS software and in JAGS. Given a model specified in the BUGS language, NIMBLE processes the BUGS code for the model and creates a model object. This model object can then be used in various algorithms, including MCMC. One of the core features of NIMBLE that distinguishes it from the BUGS and JAGS software packages is that NIMBLE provides the user with a model object that the user can manipulate. Having an explicit model object allows for the flexibility that NIMBLE provides in using, writing, and modifying algorithms to operate on a model. 
...
{\color{red} [Perry - you mentioned wanting discussion of vectorized calcs and lin algebra; do you want more than this, which was already here, except for the line at the end of this subsection?]}
...
{[\color{red} do we want to avoid the forward ref to part 3 here?]}
...
At the heart of NIMBLE's flexibility is that NIMBLE models are objects (R objects with corresponding compiled versions) that can be modified and manipulated from R. We discuss this in detail in Part \ref{part:prog}, because this usage of NIMBLE model objects is most important when writing algorithms.
...
\subsection{Working with variables and nodes in a model}
...
\subsubsection{Querying model structure}

\paragraph{Determining nodes and variables in a model}
...
\paragraph{Determining dependencies in a model}
...
\paragraph{Checking if a node holds data}
\label{sec:cdisdata}
