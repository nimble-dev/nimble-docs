...
Models in NIMBLE are specified using a variation on the BUGS language,
a language also used in the various BUGS software packages and in JAGS. Given a model specified in the BUGS language, NIMBLE processes the BUGS code for the model and creates a model object. This model object can then be used in various algorithms, including MCMC. One of the core features of NIMBLE that distinguishes it from the BUGS and JAGS software packages is that NIMBLE provides the user with a model object that the user can manipulate. Having an explicit model object allows for the flexibility that NIMBLE provides in using, writing, and modifying algorithms to operate on a model. 

This chapter describes how to specify and create model objects in NIMBLE using the distributions and functions provided by NIMBLE for use in BUGS code. With NIMBLE you can also define your own distributions and functions for use in BUGS code; discussion of this functionality is deferred to Chapter \ref{cha:user-defined} as it requires the use of nimbleFunctions. 


\section{Overview of functionality available to build models}
...
\begin{enumerate}
  \item User-defined functions and distributions -- written as \cd{nimbleFunctions} -- can be used in model code. See Chapter \ref{cha:user-defined}.
...
{\color{red} [Perry - you mentioned wanting discussion of vectorized
  calcs and lin algebra; do you want more than this, which was already
  in the manual (apart from the line at the end of this subsection
  that I added)?]}
...
includes most basic arithmetic functions and basic linear algebra. Please see Section \ref{subsec:BUGS-lang-fxns} for more details. 
What is allowed is also
described more in Chapter \ref{cha:progr-with-models} about programming with
nimbleFunctions, since BUGS code is processed similarly\footnote{However in
BUGS code there are some different rules, including the requirement to
indicate indices explicitly (or provide a \cd{dimensions} argument).  (In a
...
NIMBLE supports most of the distributions allowed in BUGS and JAGS. Table \ref{table:distributions} lists the distributions that are currently supported, with their default parameterizations, which match those of BUGS\footnote{Note that the same distributions are available for writing \cd{nimbleFunction}s, but in that case the default parameterizations and function names match R's when possible. Please see Section \ref{sec:nimble-dist-funs} for how to use distributions in \cd{nimbleFunctions}.}. NIMBLE also allows one to use alternative parameterizations for a variety of distributions as described next.
...
At the heart of NIMBLE's flexibility is that NIMBLE models are objects (R objects with corresponding compiled versions) that can be modified and manipulated from R. We discuss this in detail in Chapter \ref{cha:using-bugs-models}, because this usage of NIMBLE model objects is most important when writing algorithms.

However, in this section we provide some details on manipulating model objects that are useful when using NIMBLE's algorithms (particularly MCMC) with your models.

\subsection{Providing data and initial values to an existing model}
...
Similarly, one can set initial values for parameters via \cd{setInits}. 


\subsection{Understanding the variables and nodes of a model}

This section discusses how one can determine the names of the variables and nodes in a model, which can be useful when a user wants to choose MCMC samplers that may be different from the default samplers that NIMBLE applies to a model.
...
\subsubsection{Determining the nodes and variables in a model}

One can determine the nodes and variables in a model using \cd{getNodeNames}:
...
{\color{red} CJP asks if we need the next bit - I suspect in all cases
  we now cleanly handle any cases where a node name is given with
...
\subsubsection{Determining dependencies in a model}
\label{sec:cdgetdependencies}
{\color{red} note from 30k feet over Nebraska: check spelling descendents or descendants}

One can determine the node dependencies (or
``descendents'') in a model using \cd{getDependencies}. There are a variety of arguments to \cd{getDependencies} that
...
default \cd{getDependencies} returns descendents up through the next
...
\subsubsection{Checking if a node holds data}
\label{sec:cdisdata}
