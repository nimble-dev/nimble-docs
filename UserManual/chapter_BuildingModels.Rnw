...
TODO: WE NEED TO PUT MISSING ARGUMENT NAMES IN THIS TABLE
...
In addition, we provide the following aliases, as in JAGS: \cd{dbinom} for \cd{dbin}, \cd{dnbinom} for \cd
...
We plan to, but do not currently, handle the following distributions: double exponential (Laplace), beta-binomial, Dirichlet-multinomial, F, inverse gamma, Pareto, inverse Wishart, and various forms of multivariate t. 

% Chris: the aliases dbinom,dnbinom,dchisqr,dweibuill,ddirich are in devel and I will hopefully clean up the code so that we use an aliases object and do  direct substitution to the canonical BUGS names rather than having repeated info in distributions_inputList
% before perryEdits1 existed I had made a table of aliases - hpoefully that will get pulled in when we merge to master branch

TODO: SHOULD WE COMMENT ON MULTIVARIATE NORMAL USE OF CHOLESKY?
E.G. PROVIDE IT EXPLICITLY FOR USERS WHO WANT TO PROVIDE CHOLESKY
MATRIX DIRECTLY?
