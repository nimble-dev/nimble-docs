...
formats for models, variables, data, and initial values for MCMC.  In
addition one can create new model objects from existing model objects.
...
provided in the the \cd{constants} argument to \cd{nimbleModel}, in
...
declarations (e.g., covariates/predictors) can be provided as data or
constants.

\subsubsection{Providing data and initial values to an existing model}

Wherease constants must be provided during the call the
...
<<pump-missing, eval=FALSE, message=FALSE>>=
...
% I edited this. -PdV
Missing values may also occur in explanatory variables.  Values for
...
variables so that values are imputed by an MCMC for missing values.

\subsubsection{Defining alternative models with the same code}
\label{sec:defin-altern-models}
% THIS STAYS HERE BECAUSE IT USES nimbleModel
...
For example, an algorithm (i.e., \cd{nimbleFunction}) such as an MCMC will be
...
{\color{red} LET'S CHECK THE STATUS OF THE BUG DESCRIBED IN THE COMMENTS.}
...
\subsection{The nodes and variables in a model}
...
\subsubsection{How nodes are named}
...
\subsubsection{Why use node names?}
