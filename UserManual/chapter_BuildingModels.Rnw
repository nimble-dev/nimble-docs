...
NIMBLE makes a distinction between data and constants that allows more
...
IS THIS REDUNDANT: Missing values may also occur in variables appearing on the
...
\paragraph{Providing data via setData}
