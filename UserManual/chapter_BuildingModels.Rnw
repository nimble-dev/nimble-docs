...
Whereas constants must be provided during the call the
...
\subsubsection{What are variables and nodes?}
...
NIMBLE (Section \ref{sec:introduced-nodes}). In this case \cd{lifted\_d1\_over\_beta} is the only lifted node in the model.
