...
\endhead
   ...
   \texttt{dbin(prob, size)} & canonical \\
     ...
     \texttt{dmvt(mean, cholesky, df, prec\_param=1)} & canonical (precision) \\
     \texttt{dmvt(mean, cholesky, df, prec\_param=0)} & canonical (scale) \\
     \texttt{dmvt(mean, prec, df)} & \texttt{dmvt(mean, cholesky = chol(prec), df, prec\_param=1)} \\
     \texttt{dmvt(mean, scale, df)} & \texttt{dmvt(mean, cholesky = chol(scale), df, prec\_param=0)} \\
