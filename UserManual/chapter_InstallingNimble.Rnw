...
\cd{install.packages} or related mechanisms.  We have not yet put it on
CRAN (we expect to do so in version 0.6 in early summer 2016), so it needs to be obtained from R-nimble.org. 
See our \href{http://r-nimble.org/download}{Download page} for the specific invocation of
\cd{install.packages}.
...
\verb|R CMD INSTALL --configure-args='--enable-lib=false' nimble_VERSION.tar.gz|
...
We note that it is essential the
