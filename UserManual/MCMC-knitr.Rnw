...
An MCMC specification is created using \cd{MCMCspec()}, and the resulting MCMC specification object will generally be denoted as \cd{mcmcspec} in this Chapter.  The only required argument to \cd{MCMCspec()} is the original model object, which is the object resulting from a call to \cd{nimbleModel()}.
...
The following optional control arguments to \cd{MCMCspec()} may be used to override the default assignment of sampler algorithms:
...
mcmcspec <- MCMCspec(Rmodel, onlyRW = TRUE)
@
...
The default values of control list elements for all sampling algorithms may be overridden through use of the \cd{control} argument to \cd{MCMCspec()}, which should be a named list. 
...
mcmcspec <- MCMCspec(Rmodel, control = list(scale = 3, adaptive = FALSE))
@
...
\item Those supplied in the \cd{control} list argument in the preceding preceding call to \cd{MCMCspec()}
...
Monitors may be added to the MCMC specification either in the original call to \cd{MCMCspec()} or using the \cd{addMonitors()} method:
<<addMonitors, eval=FALSE>>=
## Using an arguments to MCMCspec()
mcmcspec <- MCMCspec(Rmodel, monitors = c('alpha', 'beta'), monitors2 = 'x')
...
<<thinning, eval=FALSE>>=
## Using an argument to MCMCspec()
mcmcspec <- MCMCspec(Rmodel, thin = 1, thin2 = 100)
...
The \cd{buildMCMC()} function accepts only a single argument, and it must be an MCMC specification originally created from a call to \cd{MCMCspec()}.  The resulting MCMC function, \cd{Rmcmc}, is an instance of a NIMBLE function specific to the model on which the specification was based.
...
The \cd{mcmc()} function has an optional \cd{reset} argument.  When \cd{reset = TRUE} (the default value), the following occurs at the onset of the call to \cd{mcmc()}:
\begin{itemize}
...
The \nm{RW\_block} sampler performs a simultaneous update of one or more model nodes, using the Metropolis-Hastings algorithm with a multivarite normal proposal distribution.  This sampler is optionally adaptive, which causes \cd{scale} to adapt throughout the course of the MCMC execution to achieve a desirable acceptance rate, and the \cd{propCov} (multivariate normal proposal covariance matrix) to adapt to emulate the empirical covariance of the sampled nodes, calculated from the MCMC samples.  This sampler may be applied to any set of continuous-valued model nodes, to any single continuous-valued multivariate model node, or to any combination thereof.
...
\item[adaptive (default = \cd{TRUE})] A logical argument, specifying whether or not the sampler should adapt the \cd{scale} and \cd{propCov} (multivariate normal proposal covariance matrix) throughout the course of MCMC execution.
...
%DT: can you make a list to include here in the manual listing what conjugate relationships we detect? Could probably wait until after the relase.
...
mcmcspec <- MCMCspec(Rmodel, monitors = 'mu')
