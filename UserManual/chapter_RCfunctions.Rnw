...
nimbleFunctions are the heart of programming in NIMBLE. In this part of the manual, you'll learn about the various ways to use nimbleFunctions. Here we'll start with simple nimbleFunctions that only contain the code to be compiled and executed, which we call the \nm{run} code. nimbleFunctions are created with the \cd{nimbleFunction} function. 
...
\item BUGS model nodes are implemented as nimbleFunctions with member
  ...
  \item nimbleFunctions cannot generally be copied safely. Instead
    rerun the \cd{nimbleFunction()} and record the result in a new object.
...
\nm{integer} and \nm{double} are supported. In addition the type \nm{logical} is
...
As illustrated in the example in Section \ref{sec:RC-intro},  the syntax for a type declaration is:
...
{\color{red} look for where we have text on indexing and see if we
  want to say something about indexing in Ch. 9; look for indexing in
  0.6-3 manuals}
...
\subsubsection{Managing dimensions and sizes, including adding and dropping dimensions}
