...
natively in R, allowing testing and debugging.  Then we compiled it and showed that the compiled version does the same thing,
...
argument.  Hence we call it the \nm{run} code.  \cd{run} code is
written in the NIMBLE language.  This is similar to a narrow subset of
R with some additional features.  Formally, it is the NIMBLE language,
which encompasses what can be compiled from a nimbleFunction.
...
\item how to declare types of arguments and return values;
...
example, help on NIMBLE's version of \cd{numeric} can be found by \cd{help(nimNumeric)}.
...
Next we cover some of these functions in more detail. {\color{red}
  What follows is reduced from some previous content.  I wonder if
...
floating-point (or ``double''), integer, or logical values, respectively.  The \cd{length} argument specifies the vector 
length (default 0), and the \cd{value} argument specifies initial 
 value either as a scaler (used for all vector elements, with default
 0) or a non-scalar (used sequentially). The \cd{init} argument specifies 
...
\cd{init = FALSE} wil be slightly more efficient performance.
...
{\color{red} check if array still requires dims to be provided using c()}
...
for changing the size of a numeric object, are given in section (\ref{xyz}).
...
\cd{length} behaves like R's \cd{length} function.  It
  ...
  each dimension.  NIMBLE's version of \cd{dim}, which has synonym \cd{nimDim}, behaves like R's
...
\paragraph*{Deprecated method of creating non-scalar objects using \cd{declare}:}
...
addition of B and C.  In NIMBLE v\ver, component-wise operations
generally only work for vectors and matrices, not arrays with more
than 2 dimensions.  The only exception is assignment: \cd{A = B} will
work up to NIMBLE's current limit of four dimensions.

{\color{red} I updated about vectorized distributions.  I still need
  to review some cases in these tables. -Perry}
...
More information on the \cd{nimbleList}s returned by the \cd{eigen()} and \cd{svd()} functions in NIMBLE can be found in Section \ref{sec:eigen-nimFunctions}.
...
from nimbleFunctions (and in BUGS model code), but care is needed in the syntax: 
\begin{itemize}
...
\item Supported parameterizations are also indicated in the list below. 
  \item For multivariate distributions,  ``r'' functions only return one random
...
{\color{red} Is that true? Do we have a system for roxygen entries
  for our version of these, or only on an as-needed basis?  Maybe one
  roxygen entry for all?} 
...
\item \cd{dexp(rate, log)} {\color{red} Is this equivalent to dexp\_nimble?
    If so can we combine rows?  I remember there was some reason for
    this situation but it is awkward.}
...
precision matrix (TRUE) or covariance
matrix (FALSE); and \cd{scale\_param} is a logical indicating whether the Cholesky is of a scale
matrix (TRUE) or an inverse scale matrix (FALSE).
...
\cd{nimbleFunctionList}.  This is described in section (\ref{XYZ}).
...
\cd{checkInterrupt()} in \cd{run} code at places where a check should
...
(floating-point), integer, logical, and character.   The \textit{type} of
...
\subsubsection{Changing the sizes of existing objects}
\label{sec:chang-sizes-exist}

{\color{red} Check if we still don't preserve contents during
  setSize. Also check if use of c() is required for correct processing.

A lot of the following example could be cut.}
...
myfun <- nimbleFunction(
    run = function(i = integer()) { ## i is an integer
...
determined to be a vector.  The following line then creates \cd{c} as
a vector, because a vector plus a scalar returns a vector.
...
matrix.  The result of matrix multiplication is always a matrix, but
...
{\color{red} If this bit is redundant with what follows, we can cut
  it.}
...
{\color{red} do we want to say anything else here about indexing? Vague memory that Perry had a vision for something more here (or perhaps somewhere else where we talk about indexing!)}
...
the expression valid, as determine at run time.  NIMBLE usually must assume during compilation
...
\section{Declarating argument and return types}
...
syntax for a type declaration is either:
\begin{itemize}
\item \cd{type(nDim, sizes)}, where \cd{type} is \cd{double},
\cd{integer}, \cd{logical} or \cd{character}; or
\item cd{nimbleListType()}, where \cd{nimbleListType} is a nimleList
  configuration created by \cd{nimbleList}.  Section \ref{nimbleLists} covers nimbleLists.
\end{itemize}

For \cd{type(nDim, sizes)}, \cd{nDim} is the number of dimensions, with 0 indicating scalar.
\cd{sizes} is an optional vector of fixed, known sizes.  Since sizes
cannot be dynamic if they are provided in the declaration, the
declaration must enclose multiple sizes in \cd{c()} if \cd{nDim} $>1$.
...
In a \cd{returnType} statement, a type of \cd{void()} means there is
no return value, which is the default if no \cd{returnType} statement
is included.  The \cd{returnType} statement can appear anywhere in a
run function.
