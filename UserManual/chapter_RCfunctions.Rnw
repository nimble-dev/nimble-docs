...
chapter, we introduce simple nimbleFunctions that contain only the
code to be executed, in either compiled or uncompiled form, but no setup function.

Simple nimbleFunctions behave like R functions: \cd{nimbleFunction}
...
when no model or modelValues is needed. These can be used for any
purpose in R programming.  They can also be used as new functions and
distributions in NIMBLE's extension of BUGS.


  Here's a basic example implementing the textbook calculation of
  least squares linear regression estimates\footnote{Of course, in general, explicitly
...
In this example, we fit 100 random response values (\cd{y}) to four columns of
...
\subsection{Flow control: if-then-else, for,  while, and stop}
...
\cd{nimbleList}s provide containers for numeric, logical, and character type variables in NIMBLE.  More information on using \cd{nimbleList}s in \cd{nimbleFunction}s can be found in Section \ref{sec:nimbleLists}.
...
If one would like to use a \cd{nimbleList} as an argument or the return value, one needs to specify the type as a \cd{nimbleList} definition (see Section \ref{nimbleLists}).
