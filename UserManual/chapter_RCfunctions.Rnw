...
\item \cd{dnorm(x, mean, sd, log)}
 ...
 \item \cd{dweibull(x, shape, scale, log)}
  ...
  \item \cd{dwish\_chol(x, cholesky, df, \hspace{4mm}scale\_param, log)}
\end{itemize}
\end{multicols}
...
## resize this numeric vector to be length 20; the last 10 elements will be 0
...
{\color{red} do we want to say anything else here about indexing? Vague memory that Perry had a vision for something more here (or perhaps somewhere else where we talk about indexing!)  
  ...
  It's basically what I added in above two subsections. -PdV}
