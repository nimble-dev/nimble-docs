...
\cd{nimbleList}s provide containers for numeric and character type variables in NIMBLE.  More information on using \cd{nimbleList}s in \cd{nimbleFunction}s can be found in Section \ref{sec:nimbleLists}.
...
If one would like to use a nimbleList as an argument or the return value, one needs to specify the type as a nimbleList definition (see Section \ref{nimbleLists}).
...
nimbleLists can be created from a nimbleList definition as discussed in Section \ref{sec:nimbleLists}. 
...
\ref{table:functions-dsl} and \ref{table:functions-matrix-dsl}.  More information on the \cd{nimbleList}s returned by the \cd{eigen()} and \cd{svd()} functions in NIMBLE can be found in Section \ref{sec:eigen-nimFunctions}.
