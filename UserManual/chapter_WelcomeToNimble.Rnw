...
As of \ver, NIMBLE has been around for a while and is reasonably
...
\item A system for writing statistical models flexibly, which is an
  extension of the BUGS language\footnote{See Section
    \ref{sec:supp-feat-bugs} for information about NIMBLE's version of
    BUGS.}.
...
We suggest everyone start with the Lightning Introduction.
